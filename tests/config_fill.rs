...
use brazen::{
    defaults, fill_absent, lead_with_preamble, AuthId, CanonicalRequest, Content, OutMode,
...
// ── lead_with_preamble: the auth-mode-required system lead (auth §4.1) ──
...
fn oauth_resolved(preamble: Option<&str>) -> ResolvedConfig {
    ...
    .or(brazen::parse_config(&toml).unwrap())
    .or(defaults())
    .into_resolved(Some("m"))
    .unwrap()
}
