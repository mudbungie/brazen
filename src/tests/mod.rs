//! In-crate relocation of the unit/integration suite (arch §9.8). These modules
//! were `tests/*.rs` integration crates; living here lets them exercise crate
//! internals through the `#[cfg(test)]` prelude in `lib.rs` WITHOUT those
//! internals being `pub` — so test layout no longer drives the semver surface.
//! Each `*_support` module is shared harness; the rest are one test file each.
//! `src/tests/` is excluded from the coverage denominator (it is the tests, not
//! the lib-under-test — Makefile `cov`), like `src/native`.

mod config_support;
mod count_support;
mod decode_full_support;
mod ingress_anthropic_support;
mod ingress_encode_support;
mod list_models_support;
mod list_providers_support;
mod login_support;
mod oauth_pure_support;
mod responses_decode_errors_support;
mod run_support;
mod serve_support;

mod ambient_discovery;
mod ambient_parse;
mod anthropic_cache;
mod anthropic_decode;
mod anthropic_decode_finish;
mod anthropic_encode;
mod anthropic_encode_content;
mod anthropic_fixtures;
mod anthropic_server_tool_fixtures;
mod anthropic_server_tools;
mod auth_apply;
mod canonical_error;
mod canonical_event;
mod canonical_event_compat;
mod canonical_ladder;
mod canonical_model;
mod canonical_request;
mod canonical_request_media;
mod canonical_request_tool;
mod canonical_v1_goldens;
mod claude_code_decode;
mod claude_code_encode;
mod claude_code_fixtures;
mod claude_code_run;
mod claude_session_conformance;
mod cli_args;
mod cli_args_prompt;
mod cli_route;
mod config_body_defaults;
mod config_defaults;
mod config_dump;
mod config_env;
mod config_errors;
mod config_fallthrough;
mod config_fill;
mod config_ingress;
mod config_partial;
mod config_preamble;
mod config_priority;
mod config_resolve;
mod config_route;
mod config_strip;
mod config_validate;
mod count;
mod count_errors;
mod cross_check_basic;
mod decode_full;
mod decode_full_structured;
mod generation_query;
mod google_decode_errors;
mod google_encode;
mod google_fixtures;
mod ingress_anthropic_decode;
mod ingress_anthropic_encode_body;
mod ingress_anthropic_encode_errors;
mod ingress_anthropic_encode_sse;
mod ingress_anthropic_errors;
mod ingress_anthropic_roundtrip;
mod ingress_openai_decode;
mod ingress_openai_encode_body;
mod ingress_openai_encode_errors;
mod ingress_openai_encode_sse;
mod ingress_openai_encode_stash;
mod ingress_openai_errors;
mod ingress_reinject;
mod ingress_roundtrip;
mod list_models;
mod list_models_errors;
mod list_models_help;
mod list_models_override;
mod list_providers;
mod list_providers_columns;
mod list_providers_errors;
mod login_browser;
mod login_device;
mod login_device_codex;
mod model_discovery_decode;
mod model_discovery_metadata;
mod oauth2_provider_recipe;
mod oauth_pure;
mod oauth_pure_callback;
mod oauth_refresh;
mod ollama_decode_errors;
mod ollama_encode;
mod ollama_fixtures;
mod oneliner;
mod openai_decode_errors;
mod openai_encode;
mod openai_encode_content;
mod openai_fixtures;
mod openai_reasoning;
mod openai_stream_terminator;
mod pipeline_input;
mod pipeline_input_media;
mod pipeline_parse;
mod pipeline_pretty;
mod pipeline_pretty_footer;
mod pipeline_pretty_write_errors;
mod pipeline_sink;
mod pipeline_style;
mod protocol_extra_fold;
mod protocol_pinned_window;
mod protocol_shapes;
mod protocol_sse_determinism;
mod protocol_sse_framers;
mod protocol_tuning;
mod provider_tools;
mod reasoning_roundtrip;
mod replay;
mod responses_decode_errors;
mod responses_decode_finish;
mod responses_encode;
mod responses_fixtures;
mod run_cache;
mod run_cache_hints;
mod run_config;
mod run_context_window;
mod run_control;
mod run_failures;
mod run_failures_stream;
mod run_failures_terminal;
mod run_files;
mod run_files_stdin;
mod run_in_filter;
mod run_in_stash;
mod run_inputs;
mod run_last_used;
mod run_learn;
mod run_modes;
mod run_raw_errors;
mod run_stream;
mod run_wire;
mod serve_control;
mod serve_listener;
mod serve_native_route;
mod serve_routes;

mod seams_config;
mod seams_protocol;
mod seams_store;
mod seams_transport;
mod service_tier;

mod transport_config;
mod transport_envelope;
mod transport_select;
mod usage_input_total;
