...
use crate::tests::decode_full_support::*;
    ...
    assert_eq!(
        ...
        vec![
            ...
            Event::Usage(Usage {
                ...
                input_total_tokens: Some(52), // 40 uncached + 8 read + 4 written (§3.2)
                ..Default::default()
            }),
            Event::ContentStart {
                index: 0,
...
fn anthropic_nonstream_refusal_forwards_stop_details() {
    ...
    let body = include_bytes!("../../tests/fixtures/anthropic_messages_nonstream_refusal.json");
    let (ev, term) = full(&AnthropicMessages, body);
    assert!(!term); // Messages' terminator is the separate `message_stop`, never in-body
    assert_eq!(
        ev,
...
fn openai_responses_nonstream_folds_reasoning_multipart_tool_and_finish() {
    ...
    let body = include_bytes!("../../tests/fixtures/openai_responses_nonstream.json");
