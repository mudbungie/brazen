...
use crate::{DecodeState, ErrorKind, Event, FinishReason, Frame, Protocol};

const ERR: &[u8] = include_bytes!("../../tests/fixtures/ollama_error.json");
...
fn whole_body_error_maps_the_status_family() {
    let err = |status: u16| -> crate::CanonicalError {
        let frame = Frame {
            event: None,
            data: ERR.to_vec(),
            status: Some(status),
