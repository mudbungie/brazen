...
/// Run `crate::login` against a fresh store and return the exit code, captured
/// stderr, and the store (to assert the persisted cred).
pub fn run(case: Case) -> (u8, String, crate::testing::MemoryCredStore) {
    let store = crate::testing::MemoryCredStore::new();
    let (code, stderr) = run_store(&case, &store);
    (code, stderr, store)
}

/// Run `crate::login` against an arbitrary store (e.g. a failing one).
pub fn run_store(case: &Case, store: &dyn CredStore) -> (u8, String) {
    let (code, _stdout, stderr) = run_store_io(case, store);
    (code, stderr)
}

/// Run `crate::login` and ALSO capture stdout — for the `--help`/`--version`
