...
pub(crate) const HELP: &str = concat!(
    ...
    "                         --dump-config omits. `shapes` is what the row's dialect can\n",
    "                         CARRY (tools, multi_turn): a row listing `-` there rejects\n",
    "                         a tool-bearing or replayed request at encode\n",
    ...
    "    --reasoning <effort> request-time reasoning effort (low|medium|high), mapped to\n",
    ...
    "    -f, --file <path>    attach a file as context (repeatable; before the prompt): text,\n",
    "                         or an image/PDF part by extension (png/jpg/jpeg/gif/webp/pdf).\n",
    "                         `-` names stdin, so `bz \"q1\" | bz -f - \"q2\"` chains two runs\n",
