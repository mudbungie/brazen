+++
title = "openai-chatgpt has no seat-independent sign-in: implement the Codex device flow (custom, not RFC 8628) and project the device capability as a --list-providers column"
created = 1788407693
updated = 1788492931
...
Filed from yog bl-c5fe (now closed there); yog's consumer ball is bl-7c9f, which bumps the pin and branches the spawn on the new column.