...
pub trait Decoder {
    ...
    /// the stream left un-terminated by its boundary char (§6.4, §7). Identity has none.
...
**The `Retry-After` header rides `TransportResponse`, NOT the `Frame`.** The non-2xx handshake also carries a second transport fact the body never holds — the `Retry-After` response header (architecture.md §3.3). It is captured on `TransportResponse.retry_after` (the one impure seam, minimal — the header verbatim, not a header map) and parsed to `CanonicalError.retry_after_seconds` (whole seconds; the `HTTP-date` form against the injected `Clock`'s `now`), then **stamped onto the whole-body error in `run`'s `response_events`** — the sibling of the model-discovery.md §5.3 404-hint enrichment, another response-level fact applied *after* `decode`. It is deliberately **not** a `Frame` field (unlike `frame.status`): `decode` needs `status` to derive the error *kind*, but never needs the header — so putting it on the `Frame` would thread a fact through a layer that does not consume it, and the clockless `decode`/`from_http_status` could not do the date arithmetic anyway. The `Frame` stays the two-envelope-fact shape (`event`, `status`); the header is carried and stamped alongside.
...
- **An unknown provider content block is DROPPED on decode, deliberately — no diagnostic channel** (the SSOT for this decision is architecture.md §3.2). A `decode` state machine that meets a native block kind it does not model leaves it **untracked** — emits no `ContentStart`, so the block's later deltas and stop fall through to `[]` — a forward-compat choice (a provider adding a block type never breaks a decode) rather than an invented `ContentKind::Other` or an error. The owned cost: the drop is invisible (no event, no warning, no counter), and `--raw` is the escape for a caller who needs the exact bytes. It mirrors the §3.2 consumer-side `Other` tolerance and is the opposite of the request edge, where an unmodeled block `type` is a hard `ParseInput`/64.
