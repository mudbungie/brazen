...
pub struct CanonicalRequest {
    ...
    pub cache: Vec<CacheBreakpoint>,    // Anthropic prompt-cache breakpoints (anthropic-messages §2.10). REQUEST-ONLY structural payload (like messages/tools): NOT config-filled, no flag, NOT stripped. ONLY the Anthropic encoder projects it to per-block cache_control; every other dialect caches by prompt prefix and ignores it. empty = no caching (the general path with empty input, never a branch). order is significant; ≤4 breakpoints, resolve-or-ParseInput (exit 64), enforced Anthropic-encode-local
    #[serde(flatten)]
...
// One prompt-cache breakpoint (anthropic-messages §2.10): WHERE to cut the prefix (anchor) and
// HOW LONG the entry lives (ttl). `anchor` is #[serde(flatten)]ed so the wire/config shape is a
// single flat object: {"anchor":"tools","ttl":"1h"} / {"anchor":"message","index":2}.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct CacheBreakpoint {
    #[serde(flatten)] pub anchor: CacheAnchor,
    #[serde(default)] pub ttl: CacheTtl,   // omitted ttl defaults to FiveMin
}

// The cut point: Tools/System anchor the whole hoisted block; Message{index} anchors a
// canonical-message index (resolved through the System-hoist skip at Anthropic encode — wire
// position ≠ canonical index). snake_case + internal tag "anchor".
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "anchor", rename_all = "snake_case")]
pub enum CacheAnchor { Tools, System, Message { index: u32 } }

// Cache lifetime. FiveMin is Anthropic's default, emitted by OMITTING ttl; OneHour emits
// "ttl":"1h". The serde renames are the one home for the "5m"/"1h" spellings on the canonical
// (config/wire) surface; the Anthropic encoder emits the literal "1h" in the OneHour arm.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub enum CacheTtl { #[default] #[serde(rename = "5m")] FiveMin, #[serde(rename = "1h")] OneHour }
```
...
- **`cache: Vec<CacheBreakpoint>` is a request-only STRUCTURAL knob, NOT a lifted cross-provider mapping.** Like `messages`/`tools` it is structural payload the caller supplies: it is **not** config-filled, has **no** flag, and is **never** stripped (it is not an `extra` key, so there is nothing to strip). It is the inverse of the three lifted knobs above: `parallel_tool_calls`/`reasoning`/`tool_choice` each map to ALL providers, but **only the Anthropic encoder reads `cache`** — it projects each breakpoint to a per-block `cache_control:{"type":"ephemeral"[,"ttl":"1h"]}` on the LAST wire block of the anchored region (tools / system / a message), while OpenAI Responses/Chat, Google, and Ollama cache by prompt PREFIX automatically and ignore `req.cache` with zero code (no marker, no strip). This deliberate asymmetry — 5 breakpoints erroring on Anthropic but passing silently elsewhere — is documented in providers.md §7. Validation (≤4 breakpoints; each must resolve to ≥1 wire block, else `ErrorKind::ParseInput`, exit 64) is **Anthropic-encode-local**, runs on the already-built `body` arrays (the SSOT for "did this region project to a wire block"), and is written BEFORE the `extra` fold so a typed `cache_control` wins over any raw one an `extra` key carries. Empty `cache` is the general path with empty input — `body` byte-identical, never a branch. The response-side `cache_read_tokens`/`cache_write_tokens` (§3.2 Usage) are a SEPARATE fact (cache HITS, not cache WRITES) and are not conflated.
