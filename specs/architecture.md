...
- **Not stateful** beyond the sanctioned exceptions: XDG config, credential/token storage, a **regenerable per-provider model-list cache** (`$XDG_CACHE_HOME`, written *wholesale* by `bz --list-models` and appended-to by the generation path's learn-on-success of one id — model-discovery.md §5, §5.4; the generation path never *lists*), and the **fail-open ingress replay stash** (`$XDG_CACHE_HOME/brazen/replay/`, ingress.md §5 — opaque reasoning-replay payloads a masquerade client's dialect cannot carry; absence degrades fidelity, never correctness, so no state is ever *required*). No history, no session files; both caches are derived-data stores whose deletion only degrades (a rebuild, a re-thought turn), never breaks.
- **No in-process fan-out — in the data plane.** One request per *pipeline* (blocking transport); the pipeline never spawns concurrent upstream requests. A caller that wants N concurrent requests spawns N `bz` — or opens N connections to the `--serve` shell, which runs N independent, unchanged pipelines on N threads (ingress.md §7).
- **No input-dialect sniffing.** Input is canonical-by-default; a non-canonical input dialect exists only where a flag or config names it **explicitly** — `--in DIALECT` / the `[ingress]` table (ingress.md) — never by structural sniffing, which stays forbidden. (This supersedes the former "no `--in-format`" absolute: the ingress capability adds explicit dialects; the no-sniffing rule is unchanged.) `--raw` on input means "these bytes are already provider-native." A **positional prompt** (`bz "…"`, §5.5) is an *explicit* alternate input channel (argv, not stdin) selected by its presence — never by sniffing stdin. When present it **wins and stdin is not read at all** (the POSIX filter idiom: read input only when needed; an unread pipe is the writer's concern via `SIGPIPE`), so there is no two-inputs error and no tty probe.
...
- **No lossless coverage of provider-unique features** in the canonical model. Logprobs, citations, safety settings ride `extra` in / `provider_detail`+`Raw` out, or require `--raw` (losing normalization). `--raw` is the one place "single representation" is knowingly bent. (Prompt-cache PLACEMENT is not such a feature and has no canonical surface: it is adapter-internal policy — the Anthropic encoder places `cache_control` marks automatically from the request's shape (anthropic-messages.md §2.10); every other dialect caches by prompt prefix with zero code. Prompt-cache ROUTING does have one, and exactly one: `cache_key` (§3.1), the opaque branch id the OpenAI family's replica pool routes by, because a stateless resend cannot reach the replica holding its own prefix without naming the branch — providers.md §7.1. The caller observes the outcome through the response-side `cache_read_tokens`/`cache_write_tokens` in §3.2; the escape from the placement policy is `--raw`.)
...
pub struct CanonicalRequest {
    ...
    pub output: Option<OutputFormat>,   // portable STRUCTURED-OUTPUT intent; the FOURTH lifted known knob (like reasoning), NOT an extra key — every dialect names JSON-mode/JSON-schema under an irreconcilable spelling each encode owns. None = plain text (the empty-set path). Exact provider structured-output objects stay in the row's body_defaults (the escape hatch, §5.3)
    pub cache_key: Option<String>,      // opaque BRANCH id for the provider's prompt cache; the SIXTH lifted known knob. OpenAI's automatic prefix cache is a replica POOL routed by prompt_cache_key, and a stateless adapter that resends the whole grown transcript lands on whichever replica takes it without one (measured: cache_read 0 on steps 3-4 of a byte-identical append-only ladder). None = absent, the key omitted and the wire byte-for-byte the pre-cache_key shape. Anthropic needs no equivalent (its encoder PLACES cache_control marks itself, anthropic-messages.md §2.10); Google/Ollama have no slot — providers.md §7.1. NO flag/env/file rung, deliberately: the value is per-BRANCH identity, and a config constant would collapse every conversation onto one key
    pub service_tier: Option<ServiceTier>, // portable processing-LANE intent (priority|standard); the FIFTH lifted known knob. Priority processing is request SHAPING, and the two families that have it spell the same lanes differently (OpenAI service_tier:"priority"|"default", Anthropic "auto"|"standard_only" — the asymmetry is providers.md §6.2). None = absent, the key omitted and the provider's default lane taken. Exact vendor lanes (flex/scale) stay in the row's body_defaults (the escape hatch, §5.3)
    pub image: Option<bool>,            // portable IMAGE-OUTPUT intent: "the reply may include an image"; the SEVENTH lifted known knob (bl-842b). One intent, two wire spellings — Google generationConfig.responseModalities:["TEXT","IMAGE"], OpenAI Responses an appended {type:"image_generation"} tool — and a loud ParseInput/64 on the dialects that return no images (providers.md §6.3). None/Some(false) = absent, nothing on the wire. The MODEL stays the caller's (-m): no model-name sniffing
    #[serde(flatten)]
...
pub enum Content {
    ...
    Document { source: DocumentSource },  // PDFs/files, INPUT-ONLY; mirrors Image exactly (one variant + one source enum). No provider RETURNS a document block → no decode side. Per-dialect reject at encode when the wire lacks a source (providers.md §9 CR-Doc)
    ToolUse { id: String, name: String, input: Value, signature: Option<String> },  // signature = Google thoughtSignature (LOAD-BEARING for Gemini multi-turn function calling); None for dialects without it
    ToolResult { tool_use_id: String, content: Vec<Content>, is_error: bool },
    Thinking { text: String, signature: Option<String>, id: Option<String>, encrypted_content: Option<String> },  // signature = Anthropic thinking signature; id/encrypted_content = OpenAI Responses reasoning-item replay — all LOAD-BEARING, all None for dialects without them
...
// DocumentSource is the Image analogue for PDFs/files — the SAME two-variant, `kind`-tagged
// shape, so a Document{Base64|Url} decodes/encodes exactly like an image. A dialect whose
// wire cannot express a given source rejects at encode (ParseInput/64), documented per
// dialect (providers.md §9 CR-Doc) — never a silent drop, never a mistranslation.
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case")]
...
pub enum Tool {
    Custom { name: String, description: Option<String>, input_schema: Value, strict: Option<bool> },  // strict = OpenAI-style strict function calling (structured-output's per-tool sibling); nested in the per-tool object each dialect spells differently (chat function.strict, Responses/Anthropic flat strict), so extra can't reach it; Google/Ollama LACK it → narrowed. None = provider default; omitted from the wire (byte-stable with the pre-knob Custom)
...
// A PORTABLE processing-LANE intent — one canonical knob, two wire spellings and two
// narrowings (providers.md §6.2). An ENUM, not a bool: "which lane" is a value, and a
// bool would be its lossy is-this-present projection (AGENTS.md). #[non_exhaustive], so
// a further rung (Flex) is additive. `openai()` = "priority"|"default"; `anthropic()` =
// "auto"|"standard_only" — Anthropic has no request-side priority DEMAND (priority is org
// provisioning), so the priority intent is "auto" and the projection is NOT injective.
#[derive(Clone, Copy, Debug, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "lowercase")]
#[non_exhaustive]
pub enum ServiceTier { Priority, Standard }

// A PORTABLE structured-output intent — one canonical knob every structured-output-capable
// dialect spells differently, lifted out of `extra` (same rule as ToolChoice / reasoning).
// Internally tagged on `type` ({"type":"json"} / {"type":"json_schema",…}) so it rides the
// wire and config the same way. `name`/`strict` feed only the dialects whose wire has them
// (OpenAI); Anthropic/Google/Ollama read only `schema` (providers.md §6). Json = JSON mode
// (no schema); Anthropic has no schemaless mode → Json is a documented narrowing there.
#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
#[non_exhaustive]
pub enum OutputFormat {
    Json,
    JsonSchema { name: Option<String>, schema: Value, strict: Option<bool> },  // name/strict skip_serializing_if None
}
```
...
- **Reasoning replay is the marquee agent-loop path, so every dialect's opaque reasoning payload has a canonical home (bl-61a9, superseding CR-A2/CR-A5/CR-R3/CR-G2's "low urgency" deferrals).** A `--json` harness running a tool loop with reasoning enabled must rebuild a replayable prior-turn transcript, which requires carrying each provider's opaque blob unmodified: (a) **Anthropic** thinking `signature` (already on `Thinking`) and `redacted_thinking` `data` (on `RedactedThinking`); (b) **Google** `thoughtSignature` — the wire attaches it to `functionCall` parts, and Gemini 2.5 multi-turn function calling **400s** if it is dropped, so it rides `ToolUse.signature` (NOT `Thinking` — it belongs to the tool call it accompanies) and Google's `encode` re-emits it on the `functionCall` part; (c) **OpenAI Responses** reasoning items — the item `id` (`rs_…`) and, when `include:["reasoning.encrypted_content"]` is requested, the `encrypted_content` blob (both on `Thinking`) — so `encode` reconstructs a `{"type":"reasoning",id,summary,encrypted_content}` input item for stateless (`store:false`) replay. Each new field is `Option`, `None` under the empty-set rule for the dialects that lack it; each adapter reads only its own field and never fabricates one. The event-surface carriage of these same facts is §3.2.
- **`Image` and `Document` are the first-class INPUT media, and `Document` mirrors `Image` exactly.** A document/PDF is a first-class content block on every major wire (Anthropic `{type:"document", source:{…}}`; OpenAI chat `{type:"file", file:{…}}` and Responses `input_file`; Gemini `inlineData`/`fileData` with `application/pdf`), so it earns a canonical variant rather than an escape hatch. `Content::Document{DocumentSource}` reuses the `Image` shape: one variant, one `kind`-tagged source enum `Base64{media_type,data}|Url{url}` — additive to the request parse (an old request without documents parses unchanged), one new decode arm, one new encode arm per dialect. **INPUT-ONLY:** no provider RETURNS a document block (documents are prompt inputs, not model outputs), so there is **no decode side** — the empty-set rule covers the response direction. Where a dialect's wire cannot express a given source, the adapter **rejects at encode** with `ParseInput`/64 (the §3.1 unrepresentable-slot rule), documented per dialect — OpenAI Chat rejects `Document{Url}` (its file inputs take no web URL, unlike `image_url`), Google rejects `Document{Url}` (the CR-G3 rule, shared with images), Ollama rejects both (no document slot at all); Anthropic and OpenAI Responses express both. The per-dialect table is providers.md §9 CR-Doc + the mapping specs. **`Image` has since grown a DECODE side (bl-0987, §3.2):** Gemini image models and the OpenAI Responses `image_generation` tool RETURN images, so `ContentKind::Image` is the response mirror of this input variant; `Document` remains input-only. **Boundary held (the ball's own scope note):** documents earn a variant, but this did NOT open a per-**block** `extra` valve — that is a bigger door (it is also where per-message name/metadata pressure would go), so it stays closed until a design pass justifies it; the top-level-request `extra` remains the only long-tail valve. **AUDIO is DEFERRED, with rationale (CR-Audio, not silence):** audio input is a real wire feature (OpenAI `input_audio`, Gemini audio `inlineData`), so an `Audio` variant is the obvious next media block — but it is **not** built here because (i) no consumer needs it yet (the empty-set rule: an unbuilt variant is the honest state, not a gap), (ii) audio carries knobs documents do not (format/sample-rate, transcription-vs-understanding intent) that want their own design pass rather than a reflexive `Image` copy, and (iii) adding it later is **additive** exactly as `Document` was (one variant + one source enum, old requests parse unchanged), so deferring costs nothing. The rationale is written so the empty set is a decision, not an oversight.
...
- **`output: Option<OutputFormat>` is the FOURTH lifted knob** (the same case as `reasoning`). Every structured-output-capable dialect names the same two ideas — "reply with JSON" and "reply matching THIS schema" — under an irreconcilable spelling: OpenAI Chat `response_format:{type:"json_schema",json_schema:{name,schema,strict}}` / `{type:"json_object"}`, OpenAI Responses `text:{format:{type:"json_schema",name,schema,strict}}` (FLAT under `format`, no `json_schema` wrapper — the one shape that differs from Chat, providers §6) / `{type:"json_object"}`, Google `generationConfig.responseMimeType:"application/json"` (+ `responseSchema` for the schema variant), Ollama top-level `format:"json"` / `<schema object>`, Anthropic `output_config:{format:{type:"json_schema",schema}}` (GA, no beta header; SCHEMA-ONLY — no schemaless JSON mode). The `extra` valve could carry exactly ONE of these spellings and the Google/Ollama ones nest or rename, so it is a TYPED field each `encode` projects, not a passthrough. `OutputFormat::JsonSchema{name,strict}` carry the OpenAI-only fields (`name` defaults to `"response"` where the dialect requires it; the schema-only dialects read just `schema`); `OutputFormat::Json` is schemaless JSON mode. **The narrowings, documented not silent (CR-R1):** Anthropic's wire LACKS a schemaless JSON mode, so `Json` is OMITTED there (like `stop` on Responses), and Google/Ollama LACK `name`/`strict`, so those fields drop. The escape hatch for a value the enum can't express stays the row's `body_defaults` (a provider-shaped `response_format`/`responseSchema`/`output_config` object rides `req.extra` verbatim), and the typed knob WINS on a same-named key through every encoder's one `extra` fold. A backend that rejects structured output lists the canonical key `output` in `unsupported_body_keys`, stripped pre-encode (config §4.1.1). **`Tool::Custom.strict` is the per-tool sibling**, lifted the same way (it too nests inside the per-tool object `extra` cannot reach): OpenAI Chat `function.strict`, Responses/Anthropic flat `strict`; Google/Ollama LACK it → narrowed. Before it was lifted a wire `strict` on a custom tool was silently dropped by the `Custom` decode — exactly the anti-pattern lifting a known knob dissolves.
- **`image: Option<bool>` is the SEVENTH lifted knob** (providers.md §6.3, bl-842b). "The reply may include an image" is ONE intent that the two image-producing dialects spell irreconcilably — Google as a generation-config modality (`generationConfig.responseModalities:["TEXT","IMAGE"]`), OpenAI Responses as a native typed tool appended to `tools` (`{"type":"image_generation"}`) — exactly the `reasoning` case (a token budget on one wire, an effort string on another). An `extra` key could carry only one of those spellings; a caller had to hand-build the request body to reach either. A **bool**, not an enum: the fact has no value beyond present/absent (the modality list and the tool's own config stay reachable verbatim through `body_defaults`/`extra` and `Tool::Provider`, and a caller-declared `image_generation` tool is never duplicated). Anthropic, OpenAI chat, Ollama and `claude_code` return no images, so `Some(true)` REJECTS there (`ParseInput`/64) — the §3.1 loud-narrowing rule, never a silent drop. `--image` never picks the model: an image-capable model is the caller's `-m`, because inferring it from a model name is the structural sniffing §2 forbids.
- **`cache_key: Option<String>` is the SIXTH lifted knob** (providers.md §7.1). Prompt-cache ROUTING is request shaping, and only the OpenAI family spells it (`prompt_cache_key`, Chat and Responses alike), so an `extra` key would carry that one spelling to whatever dialect the request was re-routed to. A `String` and not a structured id: the value is OPAQUE — brazen neither generates nor inspects it, and the provider is the authority on what it accepts. `None` = absent, the empty-set path, byte-identical to the pre-knob wire. Anthropic is a narrowing with a REASON rather than a gap: its cache is addressed by the marked prefix this adapter places itself (anthropic-messages.md §2.10), so there is no caller id to send; Google/Ollama/claude_code have no slot at all. Unlike every other lifted knob it takes **no config rung** (§5.10.3's frozen flag surface gains nothing, and `PartialConfig` no field): branch identity is per-request by definition, so a `--cache-key` flag or a file key would collapse every conversation on the box onto one key — the mis-routing the knob exists to fix. The row-level decline is the usual one (`unsupported_body_keys = ["cache_key"]`, config §4.1.1).

- **`service_tier: Option<ServiceTier>` is the FIFTH lifted knob** (providers.md §6.2). Priority processing is request SHAPING, not an extra-valve key: the two families that have it spell the same two lanes irreconcilably — OpenAI `service_tier:"priority"|"default"` (Chat and Responses alike), Anthropic `service_tier:"auto"|"standard_only"` — and `extra` could carry exactly ONE of them, so it is a TYPED field each `encode` projects. It is an ENUM, not a `priority: bool`: "which lane" is a value, and the bool would be its lossy is-this-present projection (the AGENTS.md "widen it to carry the value" rule); `#[non_exhaustive]` keeps a further rung (`Flex`) additive. **The Anthropic asymmetry, recorded not silent:** Anthropic has no request-side priority DEMAND — priority capacity is org provisioning — so the priority intent projects to `"auto"` (spend it if provisioned, else standard), which is ALSO Anthropic's default, making the projection non-injective; the ingress inverse therefore lifts only the unambiguous `"standard_only"` and leaves `"auto"` on the valve. Google/Ollama/claude_code have no lane field at all → documented narrowings (omit). A vendor lane the enum cannot spell (OpenAI `flex`/`scale`) stays the row's `body_defaults` escape hatch, and the typed knob wins on a same-named key through every encoder's one `extra` fold; a backend that rejects the key lists `service_tier` in `unsupported_body_keys` (config §4.1.1). The response-side ECHO of the served tier stays dropped — that is a `Usage`/event-vocabulary question, not this knob's.
...
- **The canonical request carries NO version handshake — it grows only additively (the §3.2 `v=1` contract, request-side).** Unlike the event stream, `CanonicalRequest` has no `v` field, and it needs none: a request is data a caller writes, brazen decodes the fields it models and forwards the rest through `extra` (above), so the request vocabulary grows exactly the way the event vocabulary does — a new capability is a new **typed field** (`reasoning`, `output`, `Content::Document`, `Tool::Custom.strict`, `Thinking.{id, encrypted_content}`, `ToolUse.signature`, …), added `#[serde(default)]`/`Option` so every prior request still parses byte-for-byte unchanged and no consumer that omits it is affected. There is nothing to version because nothing is removed or re-typed silently: a removal, rename, or semantic change to an existing field is the only breaking move, and — like an event `v` bump — it is held for the 0.1.0 freeze. Additive typed growth is the discipline; the handshake the event stream needs (a consumer parses bytes brazen produced) has no request analogue (brazen parses bytes a caller produced, tolerantly).
...
pub enum ContentKind {
    ...
    Thinking { id: Option<String> },   // id = OpenAI Responses reasoning-item id (rs_…), surfaced at block open; None for Anthropic/Google (bl-61a9). Serializes {"thinking":{}} when None (byte-compatible)
    RedactedThinking { data: String },   // Anthropic opaque blob, present AT block open (mirrors ServerToolResult's inline-content-at-start); the data is now CARRIED through the stream (bl-61a9, was empty)
    Image { media_type: String },   // a model-RETURNED image (bl-0987): the media type at block open (identity before content); the bytes follow as ImageDelta base64 fragments. Gemini image models and the OpenAI Responses image_generation tool produce it; every other dialect is the empty set
...
pub enum Delta {
    ...
    SignatureDelta(String),  // opaque signature for the block at this index (bl-61a9): Anthropic thinking `signature_delta` (folds to Thinking.signature) AND Google `thoughtSignature` on a functionCall part (folds to ToolUse.signature) — ONE grain: "the signature for block N". Wire-arrival order preserved (arrives just before the block's stop)
    EncryptedReasoningDelta(String),  // OpenAI Responses reasoning `encrypted_content` (bl-61a9): a close-adjacent opaque blob folding to Thinking.encrypted_content, emitted just before the reasoning block's stop (the wire reveals it on the output_item.done). A Delta, not a ContentStop field — the terminator stays a pure, uniform `{index}` for every block kind
    ImageDelta(String),      // standard-alphabet base64 TEXT fragments of an Image block's bytes (bl-0987): concatenate, then decode once — the json_delta assemble-then-parse discipline, never a parsed byte vector on the event
...
pub struct Usage {
    ...
    // The call's WHOLE prompt, cached slices included — the one counter whose meaning
    // does not depend on which provider answered (the containment rule below). Derived
    // by the decoder that knows the shape; None exactly when input_tokens is.
    pub input_total_tokens: Option<u32>,
    // The counters' DENOMINATOR, not a counter: the resolved model row's context window
    // (model-discovery §3, §5.5), carried in-band so a consumer needs no --list-models
    // call. No provider serves it on a generation response — decoders leave it None and
    // run::drive stamps it from the cache row. serde(default) + skip_serializing_if:
    // omitted, never null, when the row states none (grows-only, byte-identical).
...
- **`Usage` fields are `Option`, never fabricated `0`.** A provider that never reports `cache_read_tokens` leaves it `None`; `0` would be a lie ("zero cache hits" vs "unknown"). Cumulative; emitted whenever a provider reveals it. The four fields are **token-explicit** (`input_tokens`/`output_tokens`/`cache_read_tokens`/`cache_write_tokens`) — they count tokens (mapping from Anthropic `input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens` and OpenAI `prompt_tokens`/`completion_tokens`), so the names say so. The `{"type":"usage",…}` NDJSON line carries those exact keys (§5.2). **`context_window` rides the same struct but is NOT a counter** — it is the counters' denominator, the resolved model row's input token limit (model-discovery §5.5), carried in-band from the cache read `serve` already performs so a consumer that never lists still computes context fullness. No provider reports it on a generation response, so it has ONE stamp site (`run::drive::canonical_events`, the carry-the-fact rule that already governs the 404 hint and `Retry-After`), and a row that states no window leaves the key OFF the wire — `skip_serializing_if`, not `null`, so a window-less stream stays byte-identical to the pre-window shape.

  **The four counters are each provider's own number, and the providers disagree about what the prompt counter contains** (bl-d192). Anthropic's `input_tokens` is documented as "the number of input tokens which were not read from or used to create a cache", so `cache_read_input_tokens` and `cache_creation_input_tokens` sit BESIDE it — "mutually exclusive segments", and Anthropic's own docs give `total_input_tokens = cache_read + cache_creation + input_tokens`. OpenAI chat (`prompt_tokens` ⊇ `prompt_tokens_details.cached_tokens`), OpenAI Responses (`input_tokens` ⊇ `input_tokens_details.cached_tokens`) and Google (`promptTokenCount`, documented as "still the total effective prompt size meaning this includes the number of tokens in the cached content") all CONTAIN theirs; Ollama reports no cache counter at all. So `input + output + cache_read + cache_write` — the obvious total — is right on Anthropic and bills the cached slice TWICE on the other three, with the inflation growing with the hit rate: a measured 93,556-token prompt billed as 185,336, +25% over one agent tree, enough to trip a declared 2,000,000-token ceiling one model call from the answer.

  **So the decoder answers it: `input_total_tokens` is the canonical prompt total**, computed where the containment fact lives (`Usage::with_input_total`, one home, called by all five decoders) — the carry-the-fact rule that already governs the 404 hint and `Retry-After`. The consumer's rule is protocol-free: **this call consumed `input_total_tokens + output_tokens`**, and context fullness is `input_total_tokens / context_window`. The four provider counters are left byte-identical, so each still checks against that provider's own dashboard, and `cache_read`/`cache_write` still answer "how much of it was cached" — the question they exist for. On three protocols the total EQUALS `input_tokens`; that is those providers' accounting coinciding with the definition, not the field being a copy. Usage events are partial (Anthropic's `message_start` carries the prompt, its `message_delta` only the output), so a consumer merges a stream's usage events **per field, last wins**, then adds — never sums a per-event total.

  **Why additive and not a re-definition.** Two other shapes were weighed. Normalizing the three contained decoders DOWN (subtracting the cached slice out of `input_tokens`) makes the four counters disjoint slices whose sum is the consumption — one meaning, no new field — and normalizing Anthropic UP does the same in the other direction; both are cleaner end states. Both are also a **semantic change to an existing `v=1` value**, which this section's own bump rule ("`v` bumps ONLY for a removal, rename, or semantic change") sends to `EVENT_SCHEMA_VERSION = 2` and a migration for every pinned consumer. An added counter is the additive change `v=1` explicitly permits, needs no coordination, and does not foreclose either: **deferred to the `v=2` cut** — if the vocabulary is ever versioned up, fold the total into a disjoint `input_tokens` and delete this field. Documenting the per-protocol shape WITHOUT surfacing it was rejected outright: the protocol is not on the event, so a consumer could not act on the documentation.
...
- **`ContentKind::RedactedThinking { data }` mirrors the request-side `Content::RedactedThinking`.** Streamed redacted-thinking blocks open with this kind carrying the opaque `data` INLINE at `ContentStart` (the wire delivers it on the block open, exactly like `ServerToolResult`'s inline content — no streamed delta follows), so a `--json` harness can rebuild the block for replay (bl-61a9; the data was previously dropped). Adapters without the concept never emit it (the empty-set rule).
- **A model-RETURNED image is `ContentKind::Image{media_type}` + `Delta::ImageDelta` base64 fragments + `ContentStop` (bl-0987) — the response mirror of the request-side `Content::Image{Base64}`, and the fix for a silent drop.** Two dialects return images — Gemini image models (`parts[].inlineData{mimeType,data}` in the candidate, requested by `generationConfig.responseModalities:["TEXT","IMAGE"]`) and the OpenAI Responses `image_generation` tool (an `image_generation_call` output item whose `result` is the base64) — and until this ball BOTH decoders dropped them under the unknown-provider-block rule below (Google's part loop had no `inlineData` arm; Responses' `item_added` returns `[]` on an unknown item type), which violated the §3.1 loud-never-silent media rule in the one direction it did not cover. Design points: **(1) Grain.** The bytes ride as base64 *text* fragments, not an inline `Vec<u8>` at `ContentStart` — a PNG is megabytes, base64 is what both wires carry, and the fragment grain keeps each event bounded by the provider's own chunking; identity (`media_type`) is at block open, per the identity-before-content rule. Both known wires deliver the whole image in one frame, so today each block is one delta — the grain is chosen for the vocabulary, not the current chunking. **(2) Progressive renders are ignored.** Responses `partial_image` events are complete intermediate images, not appendable fragments, so they have no place in an append-only delta stream; only the final `result` decodes, and `--raw` reaches the partials. **(3) The request side IS a lifted knob — `image` (bl-842b, reversing the original ruling).** bl-0987 first held that the two wires reach image output by different MECHANISMS (a modality on Google, a tool on Responses) and so left both on existing signals; in use that forced every caller to hand-build a request body, and the reasoning was wrong by this spec's own standard — `reasoning` is also one intent with mechanically different spellings. `--image` / the typed `image` field now projects per dialect (providers.md §6.3); the raw spellings stay reachable. **(4) Replay is a user turn.** The fold recipe (canonical-protocol §3.3) rebuilds `Content::Image{Base64}`; a caller replays it as USER input (`-f bz-<hash>.png`), the stateless editing pattern each producing dialect documents — no dialect's assistant slot is promised to carry it, and none is changed here. **(5) No `id`.** The Responses item id (`ig_…`) is a stored-state handle (`store:true` multi-turn reference); brazen's replay is stateless, so the variant carries none — additive if a consumer ever needs it, exactly as `Thinking.id` was. **Additive under `v=1`** — one kind, one delta; a pinned consumer routes both to `Other`. `Document` stays input-only (no wire returns one). Text-mode rendering is §5.3.
- **Reasoning-replay payloads ride the event surface so a `--json` harness can rebuild replayable transcripts (bl-61a9, the §3.1 request-side facts carried on the stream).** Three opaque provider blobs travel by the grain that matches their wire arrival: (1) an **opaque signature** for a block arrives as `Delta::SignatureDelta(String)` — Anthropic emits it as the `signature_delta` just before a thinking block's stop; Google's `functionCall` `thoughtSignature` (a block-open part property) is surfaced as this same delta on the tool block. A sink folds it onto the open block's signature slot (`Content::Thinking.signature` or `Content::ToolUse.signature`) — ONE grain for "the signature for block N", so `ContentKind::ToolUse` is unchanged. (2) The **OpenAI Responses reasoning-item id** rides `ContentKind::Thinking { id }` (identity known at block open, like `ToolUse.id`). (3) The **Responses `encrypted_content`** rides `Delta::EncryptedReasoningDelta(String)`, emitted just before the reasoning block's stop (the wire reveals it on `output_item.done`); it folds onto `Content::Thinking.encrypted_content`. Carriage decision (§3.2 grain): `encrypted_content` is a Delta, **not** an optional `ContentStop` field, so the terminator stays a pure uniform `{index}` for every block kind. **All additive under the `v=1` contract** — two new `Delta` variants and two new struct-variant fields (each `Option`, omitted-when-`None` so existing bytes are unchanged); no `EVENT_SCHEMA_VERSION` bump (a pinned consumer routes an unknown delta to `Delta::Other` and ignores an unknown field).
...
- **An unknown PROVIDER block is silently DROPPED on decode — the forward-compat tolerance turned toward the wire, deliberately and with no diagnostic channel.** The `v=1` `Other` tolerance above is the *consumer* edge (an unknown value in brazen's OWN NDJSON). The *provider* edge is its mirror: when a `Protocol::decode` state machine meets a native content-block type it does not model, it leaves the block **untracked** — emits no `ContentStart`, so the block's later deltas and stop fall through to `[]` — rather than inventing a `ContentKind::Other` or raising an error. This is intentional: a provider that adds a new native block type must never break a decode, and brazen has no faithful canonical rendering of a block it has never seen (unlike its own vocabulary, where `Other` carries the verbatim `{tag: body}`). The owned cost, stated plainly: the drop is **invisible** — no event, no warning, no counter, no `provider_detail` note — so a genuinely meaningful new block is lost without a signal until brazen learns it; the escape for a caller who needs the exact bytes is `--raw` (§5.4). It is the OPPOSITE of the request edge, where an unmodeled content-block `type` is a hard `ParseInput`/64 (our own input is validated; a provider's output is tolerated), and it is the drop the sse-decoder spec's decisions list also owns (sse-decoder §12).
...
There is **no model→provider routing table** (a second home would drift). Resolution is a **query over the rows**, computed once during config resolution: the user names a provider explicitly (`--provider anthropic`) **or** brazen takes the **first row, in priority order, that owns** the model. A row owns the model on either of **two tiers**: it **claims** the model — its `model_aliases` spell it (substitution shorthand) **or** one of its `model_prefixes` claims its family (e.g. anthropic owns `claude-`, openai owns `gpt-`/`o1`/`o3`/`o4`/`chatgpt-`) — or, when *no* row claims it, its **observed model list places it** (§4.3.2). Within a tier either kind of match is enough, and all of it feeds the one query. Alias→wire-id substitution happens **in resolution**, so `ProviderCtx.model` is already the wire id and `encode` has no model logic.

**The config is a priority list, and routing is a greedy first match over it.** Rows are ordered by **declaration** — config-file order, top to bottom, a user file's rows ahead of the built-in defaults (§4.3.1) — and the first owner wins. Several rows *may* own one model; that is not an error and needs no tiebreak, because a total order always has a first element. So an exact `model_aliases` spelling on an earlier row beats a later row's `model_prefixes` family claim, and this is what makes the one-line masquerade work: aliasing `gpt-4o` on the first-declared `anthropic` row diverts that one id to Claude while `openai` keeps serving every other `gpt-…` by prefix ([ingress.md](ingress.md) §6). The rule is uniform — it never asks *which kind* of claim matched, only *which row came first*.

This **retires the former `AmbiguousModel` error** (two owners → 78, "never a silent pick"). That rule bought less than it cost: the only way to break a tie without an order is to refuse, and refusing forced the masquerade recipe to *disarm a whole vendor's family routing* (`model_prefixes = []` on `openai`) to divert a single id — collateral far worse than the ambiguity. Two owners is now the operator's own list saying which they meant.

**This is not a new stance — the same shape of rule already governs one layer down.** Model *selection* faced its own many-candidates question and ruled this way: "**List order is authoritative.** Providers return newest-first (Anthropic) or creation order (OpenAI); the *first* match is 'the suggested version.' **No ambiguity error — the order IS the tiebreak**" ([model-discovery.md](model-discovery.md) §4). Routing was the outlier, refusing where selection picked; both now say *an ordered list needs no ambiguity error*.

The **rhyme is real, the questions are not identical**, and the difference is the interesting part: selection's order is the **provider's** (newest-first — an authority claim about versions, inside one vendor), while routing's is the **operator's** (an authorship claim about intent, across vendors — and so across *credentials*, which is why [auth.md](auth.md) §5.2 must warn that an account-scoped row claims nothing and selection needs no such warning). Whose list it is decides what the order means. The layer boundary itself is unmoved: order tiebreaks *within* each step, and a partial model still cannot pick a provider (below). **This is the one place a config-order-dependent outcome is deliberate**; everywhere else order is discarded precisely so it cannot matter.

**The honest cost: the pick itself is not surfaced.** Under `AmbiguousModel` an overlap announced itself; now the earlier row wins in silence. `--dump-config` shows the **order** — it does not show *which row won for model X*, which still means reading the ownership rule across the list yourself, and brazen adds no `--why-this-row` to tell you — §2 is explicit ("**No verbosity/`--debug` flag.** Diagnostics ride the in-band error's `provider_detail`"), and a routing-explainer verb is exactly the mechanism this ruling exists to avoid. We take that cost knowingly, the way §3.2 takes the silent unknown-block drop: the mitigation is `--provider`, which overrides routing outright for anyone who does not want to reason about priority at all.

#### 4.3.1 Priority order is declaration order — and the row table *is* the list

Ownership needs a **total order over rows**, and the honest one is the order the operator wrote. Priority is: **each layer's `[[provider]]` rows in file order, the higher-precedence layer's ahead of the lower's** — so a user file's rows outrank the embedded defaults', and within a file the top row wins. This is not a new rule; it is the *general* form of the one §4.3's zero-config default already used ("whatever you find first, reading the config from the top," a user file's first row outranking the built-ins'). That rule only ever materialized the order's **head**; routing now uses the whole list.

So the order is not stored beside the rows — **the rows are stored in it**. `PartialConfig.providers` is an ordered `Vec<(String, PartialProvider)>`, not a name-keyed map, and every part of this ruling is then a plain read of that list: greedy-first routing is `.find(row_owns)`, the zero-config default is `.first()`. The old **`default_provider` field dissolves** (it stored the head of a list that now exists) — computed, not stored, the §10 rule. One fact, one home, and no second structure to fall out of agreement with it (config.md §2.2 carries the full argument, including why an `IndexMap` and an order-vec-beside-a-map were both rejected).
...
**Prefix ownership is what makes `--provider` droppable for an unmistakable id** (`bz -m claude-haiku-4-5-20251001 "q"` routes to anthropic with no flag): a versioned wire id no alias table could ever enumerate is routed by the *family* its row claims. Ownership covers *routing* only, distinct from alias *substitution* — a model no row owns **on either tier** (no alias, no prefix, and no cached list that can place it) still requires an explicit `--provider`. Two rows that serve the same family stay opt-in: `openai-responses` ships **no** `model_prefixes` precisely because it serves the same OpenAI ids as `openai` over a second protocol — were it to claim them, greedy-first (§4.3) would resolve every `gpt-…` by the *defaults' own declaration order*, silently picking a protocol the operator never chose. A shipped default must not encode an accidental priority between two rows serving one id set; claiming nothing keeps the second protocol reached the one honest way, by explicit `--provider`. (`ollama`'s local model names have no stable family, so it too stays explicit.) Greedy-first makes overlap *legal*, not *free* — it is a tool for an operator ordering their own list, not a licence for the defaults to guess.

**Imprecise models resolve against the model cache (no probe).** Ownership answers *routing* — which row to send to. It no longer also decides *is this a full wire id?*: every model string (full, partial, or absent) is resolved in `serve` against the provider's **cache** by the total `select_model` (model-discovery.md §4–§5) — an exact match, else a partial (first listed id containing it), else the **default** (absent model → first listed), else the string **verbatim** (cache miss/no match → attempt it literally). This is a **local file read, not a round-trip**; there is no probe and no `ResolvedConfig.probe`. **It relaxes routing exactly as far as the cache already reaches** — §4.3.2, which narrows a prior ruling stated here. That ruling read: "*a given partial still cannot pick a provider (that would be a vendor-name table or an N-provider fan-out, both forbidden), so `bz -m opus "q"` with no `--provider` and no configured `provider` is still `NoProvider` (78)*." The *forbidden* half stands untouched — brazen still has no model→vendor table and still fans out to nobody — but the conclusion drawn from it was too wide, and §4.3.2 argues why. Full mechanics: [model-discovery.md](model-discovery.md).

**With NO model and NO provider, resolution defaults to the first DECLARED row.** The zero-config `bz "q"` (no `--provider`, no `--model`/`BRAZEN_MODEL`) is *not* `NoProvider`: routing falls back to the **first provider declared in config-file order** — "whatever you find first, reading the config from the top" — and the empty model seed walks `select_model`'s ladder (model-discovery.md §4.1: the model that row **last used**, else its **first cached model**) — a *model*-selection rung only; provider routing has no last-used and stays config order + filtering. Crucially this is *declaration* order, **not** the alphabetically-first name — it is simply the **head of the priority list** routing already runs on (§4.3.1), i.e. `providers.first()`, not a second fact. A **user file's rows outrank the built-in defaults'** — so a config that declares `chatgpt` first defaults to `chatgpt`, never the built-in `anthropic` it sits beside in the merged table. Symmetric defaults: "no specification" → (first declared provider, that provider's model ladder), the empty-input dissolve of the routing special case. This is distinct from `bz -m opus "q"` above — a *given* model no row owns on either tier still cannot pick a provider, because *guessing* one for a named model would need the forbidden vendor table; with *no* model there is nothing to guess against, so the first declared row is the honest default. **An empty model string is absence, not a seed** — `--model ""` / `model = ""` resolves here, not to `NoProvider`, the same empty-input dissolve `cfg.model = ""` already spells downstream. The lone residue of `NoProvider` on the no-model path is a config with **no provider rows at all** (an empty priority list has no head to fall back to). **`--login` is the exception** — a credential write must name its target, so it requires an explicit `--provider`/configured `provider` (the *selector*, not this default) and never takes it ([auth.md](auth.md) §7.1).

#### 4.3.2 The second ownership tier: a row also owns what its cache has *observed*

**Tier 1 is what a row CLAIMS; tier 2 is what a row has been OBSERVED to serve.** When no row claims the routing model, routing reads the list a second time and the first row whose **cached model list can place the seed** — under `select_model`'s own rule, exact id else first case-insensitive substring ([model-discovery.md](model-discovery.md) §4) — owns it. So with `anthropic` declared above `openai`, `bz --model 5.5 "q"` routes to **openai**: no row claims `5.5`, anthropic's list cannot place it (`5.5`, with a dot, is not a substring of `claude-sonnet-5-5`), openai's can (`gpt-5.5`). Before this it was `NoProvider` (78) with both providers configured and one of them plainly serving the model.

**Why this is not the forbidden vendor-name table.** The prohibition in §4.3 is against a *central model→vendor mapping* — a second home for a fact, which would drift — and against *fanning out* a request to N providers to see who answers. The cache is neither:

- **It is per-row and first-hand, not central and authored.** There is still no table anywhere that says "`gpt-…` means OpenAI". What exists is, per row, the list *that row's own endpoint returned* to `bz --list-models` under that row's credentials. The provider is the source of truth about its own catalogue; brazen stores an observation, not an opinion. Delete the row and its knowledge goes with it — there is no leftover mapping to fall out of agreement with anything. A vendor table is a fact brazen would have to *author and maintain*; this is a fact brazen was *told*.
- **It never fans out.** Routing performs **zero** network operations. Tier 2 is a local file read per row, short-circuited by the same greedy `find` — one more pass over one list, then exactly one request to exactly one row, as before.
- **It adds no mechanism to remove.** No flag, no config key, no verb, no new matching rule. Severability is total in the strongest form: `rm -rf $XDG_CACHE_HOME/brazen/models` and behaviour degrades *precisely* to the tier-1-only rule this section narrows.

**And the honest framing: this subtracts a disagreement rather than adding a capability.** Routing and selection were already answering the *same question* about the *same string* against the *same rows* — and disagreeing. Selection said `opus` means `claude-opus-4-1`; routing said `opus` means nothing at all, so 78. The seam between the two stages was the only reason a seed could be *placeable* and *unroutable* at once. Tier 2 is that seam removed: one rule about what a model string means, applied once for the row and again for the wire id.

**Tier precedence is whole-list, not row-at-a-time: every row's claim before any row's cache.** A claim is the operator's declaration; a cache entry is an observation, and observations can be stale or incidental. In particular `generate` **learns on success** ([model-discovery.md](model-discovery.md) §5.4): a 2xx on a model the cache could not place appends that id to the routed row's list. Were the tiers interleaved, one `bz --provider anthropic --model gpt-5.5` that happened to succeed (against a gateway row, say) would append `gpt-5.5` to a row declared *above* `openai` — and silently divert the entire `gpt-` family from the row that **claims** it, permanently and with no diagnostic. Walking tier 1 across all rows first makes that unreachable: **a learned or stale entry can only ever win a model that no row claims at all**, which is exactly the gap the tier exists to fill. Within each tier the greedy first-match of §4.3 is unchanged, and §4.3.1's declaration order is still the only tiebreak.

**The cost, stated plainly: a cold row cannot win tier 2, and routing will not probe to fix that.** A provider brazen has never listed has no observed models, so fall-through past it is invisible until `bz --list-models` has run for it. Probing would put network I/O — latency, auth, and failure — inside config resolution, which §4.3 keeps a pure local computation; that trade is refused. The `NoProvider` message names the remedy (run `bz --list-models`), and `--provider` remains the unconditional override. This also inherits §4.3's known cost: the pick still is not surfaced, and brazen adds no `--why-this-row` to explain it.
...
if !raw {
    ...
    cfg.model = wire;
    cfg.model_from_cache = matches!(prov, Provenance::Cached);        // carried for the 404 hint (model-discovery.md §5.3)
}
...
{"type":"usage","input_tokens":12,"output_tokens":2,"cache_read_tokens":null,"cache_write_tokens":null,"input_total_tokens":12}
...
**An `Image` block in text mode is a FILE, never stdout bytes (bl-0987).** Binary on stdout would corrupt every filter the §5.3 contract exists for (`bz "q" | tool`, `$(bz "q")`, `> file`), and handing the bytes to a system opener (`xdg-open`) would be an impure side effect the lib cannot test and a fork of the unix-filter model. So the sink accumulates the block's `ImageDelta` fragments and, at its `ContentStop`, decodes the base64 once and writes the bytes to **`<dir>/bz-<sha256 hex[..12]>.<ext>`**, then prints that path as one line on **stderr** (the curl idiom: body on stdout, meta on stderr; the same channel `Event::Error` already uses in text mode). stdout is untouched — the model's words, if any, still stream there byte-identically. **The name is content-addressed**, which dissolves every alternative: no clock (impure), no counter (state), no `--out` flag (a new flag is a smell, and `--json | jq` already reaches the base64 for a caller who wants to name it), no `create_new` refusal (the same image is idempotent, a different image never collides), no message id (Google streams none). `<ext>` derives from the media type through the ONE extension↔media-type table `-f` already owns (input.rs `MEDIA_TYPES`, read in both directions — one fact, no second table; a type with two spellings takes its first row, so `image/jpeg` → `jpg`); an unmapped type is `bin`. **`<dir>` is the seam:** the sink takes a directory; `run` passes `"."` (the OS resolves it — the lib never reads the working directory), tests pass a tempdir, so the write is covered without touching the crate tree. The write is mode-invariant within text mode (PLAIN and pretty both write; the file is the answer, not chrome — interactive-output §5) and absent from `--json` (base64 inline) and `--raw` (provider bytes). A failed write is an `io::Error` like any stdout write failure and maps through `pump` unchanged.

**`--tier <priority|standard>` is the processing-LANE request knob** (§3.1's fifth lifted knob, providers.md §6.2): it resolves into the canonical typed `req.service_tier` and is projected per-protocol at `encode` (OpenAI `service_tier:"priority"|"default"`, Anthropic `"auto"|"standard_only"`; Google/Ollama/claude_code narrow it away). It folds like any gen knob — request > `--tier` flag > `BRAZEN_TIER` > config-file `service_tier = "priority"` — and the flag/env use the operator's short word while the field, the file key and every wire spelling use `service_tier`.

**`--image` is the IMAGE-OUTPUT request knob** (§3.1's seventh lifted knob, providers.md §6.3): a bool resolving into the typed `req.image`, folded like `--tier` (request > `--image` > `BRAZEN_IMAGE` > config-file `image = true`) and projected per protocol at `encode`. What comes back is an `Image` block, which `--text` writes to a file (below) and `--json` carries inline.
...
**`-f` / `--file <path>` — repeatable content-attach (the constructor's file ingredient).** The `--input`-as-list door above stays shut, but it pointed at a real need it was the *wrong shape* for: meeting a caller who simply has **a couple of files to feed in as context**. The right shape is **not** "a list of canonical-request documents" (no single merge, above) but **content-attach**: `-f path` is **repeatable** (`flags.files: Vec<PathBuf>`, **accumulating** — *not* last-wins, the one place this differs from `--input`), and each named file's whole contents become **one content part** — `Content::Text`, or a media part when the extension is in the closed table below. The parts are **context preceding the prompt, in one user message**: the constructed user turn is `[file₁ part, …, fileₙ part, positional-prompt text]` — files first (argv order), the prompt last. So `bz -f notes.md -f data.csv "summarize these"` sends one user message of three text parts, and `bz -f photo.png "what is this?"` an image part plus the prompt. `-f` is a **constructor ingredient**, exactly like the positional prompt — `read_request` assembles the one `CanonicalRequest` from `(files, prompt)` and config/flags fill `system`/`model`/gen-params via `fill_absent`. It is **distinct from `--input`** and reconciles with it, never contradicts: `--input` *replaces* the whole request document ("the caller assembled the request"); `-f` *builds* the one user message ("the caller has files and maybe a prompt; brazen assembles the request"). This is the clean reframe the `--input`-as-list decision pointed to — content-attach was the answer, a request-document list was not.

**`-f -` names stdin — the portable spelling of a capability that already existed.** A `-f` path of `-` reads **stdin** into the same one `Content::Text` part a named file yields, so `bz "name one dish" | bz -f - "how do I make this?"` chains two runs. This is deliberately **not a new capability**: `-f` takes a path, `/dev/stdin` *is* a path, and `open_input`'s file-vs-pipe parity meant `-f /dev/stdin` already worked — but only on POSIX, and brazen ships `x86_64-pc-windows-msvc` and `aarch64-pc-windows-msvc`, where no such path exists. `-` is the portable, conventional spelling (`cat -`, `tar -f -`, `sort -`), and it is an explicit value **naming** stdin, exactly like a filename — *not* an absent value with a fallback, which is why it composes with the repeatable form under no extra rule. It reads the process's real stdin, never the `--input FILE` reader: `--input` redirects the *request channel*, `-` names the *stream*. Reading it **exhausts** it, so a subsequent canonical parse sees `Ok(0)` — the caller who asked for stdin as text got it as text, and the `-f`-plus-piped-request refusal below cannot fire on bytes already claimed. **No special cases:** `-f - -f -` reads stdin once and the second `-` hits EOF, yielding the empty part `-f empty.txt` already yields; an interactive tty is the shim's `io::empty()` (below), so `bz -f - "q"` at a terminal is an empty part, never a hang; and a non-UTF-8 or unreadable stdin folds into the same exit **66** as a bad file, reported against the `-` path. Crucially this is **opt-in**, so `bz "q"` still never reads stdin — brazen does not join the set of commands that eat a `while read` loop's input (the `ssh`-needs-`-n` hazard). Making a bare positional prompt consume stdin implicitly was considered and **deferred**: it would reverse the positional-wins ruling below and introduce exactly that footgun, whereas `-f -` stays valid and harmless if the implicit default is ever adopted.
...
**`-f` media-detection — the extension names the part type.** A `-f` path whose extension is in the **closed media table** attaches as the matching canonical media part instead of text: `.png`/`.jpg`/`.jpeg`/`.gif`/`.webp` → `Content::Image{Base64}`, `.pdf` → `Content::Document{Base64}` (§3.1) — the file read as **bytes** (`fs::read`), standard-alphabet base64, the `media_type` from the table (case-insensitive extension match). The variant family is **derived from the media type, not stored twice**: `image/*` → `Image`, anything else → `Document` — one table, one fact. The signal is the **path the caller typed, never magic-byte content sniffing** — the same explicit-signal rule that forbids input-dialect sniffing (§2): a PNG misnamed `notes.txt` reads as text and fails UTF-8 (exit 66) rather than being silently reclassified, and a text file misnamed `x.png` ships as an image for the provider to reject — the caller's name is honored both ways. The table is deliberately **minimal**: exactly the base64 media shapes every routed dialect has a documented projection (or documented reject) for in providers.md — a local file is always `Base64`, so the URL-source narrowings (Gemini/Ollama) never arise, and per-dialect capability stays the *encoder's* published rule, not detection's (`bz -f doc.pdf` routed at Ollama is the encode-time document reject, exit 64; detection is dialect-blind). Everything unmapped — no extension, an unknown extension, and `-` (stdin names a stream, not a suffix) — is the **text path, byte-for-byte unchanged**. **No new flag** (the `Vec<PathBuf>` surface was forward-compatible by design, the door this paragraph's predecessor left open; bl-956c added the canonical variants + per-dialect encodes, bl-b8ef added this detection), and **no size guard** — the provider owns its payload limits and its 4xx already surfaces.

**Errors — like `--input`.** A missing, unreadable, **or non-UTF-8** `-f` file is exit **66** (`EX_NOINPUT`), the same class as `--input`: on the text path one `std::fs::read_to_string` per path folds all three failure modes into one `io::Error` (a text part is UTF-8, so a non-UTF-8 file "was not readable" *as the text it must be*); on the media path `fs::read` folds the two that exist (bytes have no encoding to violate). Both surface on **stderr** before any sink exists (the §5.9 pre-sink fatal region, sibling of the `--input` open), with a message naming the offending path. `-f` composes **orthogonally** with `--system`/`--model`/the gen-params (they shape different fields).
...
bz --skill                                # print the fuller skill doc (worked examples), exit 0
...
- **`--list-providers`** is the read of the **effective** provider table (config.md §6.1, bl-b679) — the local sibling of `--list-models`, which lists one row's *models*. It resolves **no** provider and routes to none: it folds the config the way a run does (`flags.or(env).or(file).or(defaults())`) and completes **every** row through the one `row::complete` lift, printing `name` / `protocol` / `auth` / `tuning` / `credential` in `providers` order, so the listing's order **is** routing priority and its head **is** the zero-config default. It is the one listing that is **offline by construction** — it takes stdout/stderr/`CredStore` and no `Transport` at all, so zero round-trips is a property of its type. It exists because `--dump-config` deliberately drops the defaults operand (§6.2, config.md §6) and therefore can never show the embedded rows; `--list-providers` keeps that operand because it writes no file. `credential` is `resolved_secret`'s answer minus the network (`not required` / `inline` / `stored` / `ambient` / `missing`), so it cannot disagree with what a run would do. The `tuning` pair (`effort`/`priority` — two booleans under `--json`, one comma-joined column in text) is the same kind of fact for the two request knobs: the dialect's `Protocol::tuning()` declaration AND the row's `unsupported_body_keys` decline, the very pair `strip_unsupported` enforces, so a consumer above brazen keeps no copy of the protocol table (config.md §6.1, bl-50a5).
...
- **Mutual exclusion / precedence.** The three *probes* (`--help`, `--skill`, `--version`) answer before any config or network and win first (`--help` > `--skill` > `--version`) — a probe must respond even with a broken config. The control operations (`--dump-config`, `--list-models`, `--list-providers`, `--login`, `--count-tokens`, `--serve`) are otherwise mutually exclusive; combining two is a usage error (64).
...
#### 5.10.2 `--raw` is directional (`--raw=in` / `--raw=out`), bare `--raw` = both

`--raw` is the single, knowingly-bent place where normalization is skipped (§5.4). It has **two independently-toggled halves** — the verbatim REQUEST in (`parse`+`encode` skipped: the stdin body goes to transport verbatim, the positional prompt and `-f` unused) and the verbatim RESPONSE out (`decode`/normalize skipped: transport bytes stream back as `Event::Raw`). It is "brazen as a dumb authenticated pipe" per half, sharing the encode/auth/transport/status middle with the normalized path (§5.4).

**Value grammar** (bare `--raw` and `--raw=<dir>`; an unknown value is a usage error, 64):

| Spelling | Sets `OutMode` | Input-axis override | Meaning |
|----------|----------------|---------------------|---------|
| `--raw` (bare) | `Raw` | — (derives from `OutMode`) | verbatim in **and** out |
| `--raw=both` | `Raw` | — (derives from `OutMode`) | the explicit spelling of bare `--raw` |
| `--raw=in` | *(unset)* | `raw_in = true` | verbatim request in, **canonical** events out |
| `--raw=out` | `Raw` | `raw_in = false` | **canonical** request in (constructor/stdin, full parse+encode), verbatim response bytes out |

The two facts are **`OutMode == Raw`** (the OUTPUT axis — the `RawSink`) and **`raw_in`** (the INPUT axis — send the stdin body verbatim). Resolution derives `raw_in = raw_in_override.unwrap_or(OutMode == Raw)`. This single derivation is what makes bare-`--raw`'s "both" fall out for free (its `OutMode` is `Raw`, so the input axis derives `true`) **and** keeps the split backward-compatible under the last-wins `OutMode` fold (below).

**The four input×output combinations** (the request half toggles on `raw_in`, the response half on `OutMode == Raw`, independently):

| `raw_in` | raw out | invocation | request half | response half | terminator | exit source |
|----------|---------|------------|--------------|---------------|------------|-------------|
| F | F | *(default)* / `--text` / `--json` | constructor → `encode` | canonical events + one `End` | `End` (NDJSON) / EOF (text) | in-band `Event::Error` |
| F | T | `--raw=out` | constructor → `encode` | verbatim bytes (`RawSink`) | the provider's own | peeked status |
| T | F | `--raw=in` | verbatim body → wire | canonical events + one `End` | `End` (NDJSON) / EOF (text) | in-band `Event::Error` |
| T | T | `--raw` / `--raw=both` | verbatim body → wire | verbatim bytes (`RawSink`) | the provider's own | peeked status |

**Per-half rules** (the §5.5 composition, re-derived per direction): *raw in* keeps stdin-verbatim semantics — the constructor never runs, so `-f` is **refused** (64, `--file cannot be combined with --raw`) and a positional prompt is **unused** (the verbatim stdin body is the request); the refusal keys on `raw_in`, so `--raw=out` (constructor runs) composes with `-f` normally. *Raw out* keeps the §5.4 output rules — the provider's terminator stands, brazen injects **no** `End`, the `RawSink` flushes per chunk. *Canonical out* (`--raw=in`) emits the one `End` (§5.6) and the model cache is still bypassed on the request half (no positional/model routing on verbatim bytes; the §5.3 model-provenance hint is therefore absent). The **raw-4xx/5xx-never-exits-0 rule holds in every combination** (§5.4).

**`OutMode` interaction — one last-wins fold, no new conflict path.** `--raw=out` **IS** an output mode: it sets `OutMode = Raw` and participates in the last-wins `OutMode` fold exactly like `--text`/`--json`/bare `--raw` (§5.10.3), so `--raw=out --json` ⇒ `Ndjson` (json wins — normalized both ways) and `--json --raw=out` ⇒ raw out (normalized in). `--raw=in` does **not** touch `OutMode` — it is the input-axis flag — so it *composes* with the projection: `--raw=in --json` ⇒ verbatim request in, canonical NDJSON out; `--raw=in` alone ⇒ verbatim request in, `--text` projection out. Because bare-`--raw`'s input rawness **derives** from the final `OutMode`, `--raw --json` ⇒ normalized json (the later `--json` moved `OutMode` off `Raw`, so the derived input-rawness lapses) — the pre-split meaning, unchanged. An explicit `--raw=in --json` keeps its input rawness (the override survives, being input-axis independent). A config-file/env `output = "raw"` carries no direction and so means **both** — the symmetric meaning `--raw` has always had — because the derivation reads it as `OutMode == Raw` with no override.

**Provenance.** The split was deferred at 0.1.0 as the rare CLI change here that is *not* a one-way door (bare `--raw` means "both" today and forever, so the split is a pure backward-compatible extension); the owner pulled it forward (2026-07-08, "sure, I can't think of a strong reason not to do this") for the compelling **normalized-in / raw-out** case — the ergonomic request with the exact provider wire bytes out, the only encode-observability window (§2). No existing `--raw` invocation changes meaning (§13.14).
...
| `--timeout <s>` | the **silence budget** in whole seconds: abort when the upstream makes no progress (sends no bytes) for `s` seconds, applied per phase — connecting, awaiting the response headers, and between body chunks (§13.15). It is **not** a wall-clock *total*: a long-but-live stream never trips it (the timer resets on every byte), and a total-duration knob is separately rejected (§13.15). One value; internally it fans onto ureq's connect + response-header + inter-chunk-idle budgets (mechanism, config §4.3), not three surface knobs |
...
| `--tier <priority\|standard>` | request-time processing LANE; the portable intent each protocol maps to its own `service_tier` spelling (providers.md §6.2). Omitted = the provider's default lane |
| `--image` | the reply may include an image (the SEVENTH lifted knob, §3.1): Google adds `responseModalities:["TEXT","IMAGE"]`, OpenAI Responses appends the `image_generation` tool, the other dialects reject with 64 (providers.md §6.3). Pick an image-capable model with `-m` |
...
| `--raw` / `--raw=both` | provider-native passthrough, both directions (§5.4, §5.10.2) — sets `OutMode = Raw`, input-rawness derived |
| `--raw=out` | verbatim response bytes out; canonical (constructor-built) request in — sets `OutMode = Raw` (participates in this last-wins fold), input normal |

**Input-axis flag** (orthogonal to `OutMode` — does **not** set the output mode, so it composes with the projections above; §5.10.2): `--raw=in` — verbatim provider request in, canonical events out under whatever `--text`/`--json` is set. `--in DIALECT` (ingress.md §11) is the third input contract — one client-dialect request on stdin, the dialect response on stdout — mutually exclusive with a positional prompt, with `--raw=in`, and with `-f` (64); it composes with `--raw=out`.

**Input source flags:** `-f`/`--file <path>` (**repeatable**, accumulating — each named file's contents become one content part — text, or `Content::Image`/`Content::Document` when the extension is in the §5.5 media table — prepended in argv order before the positional prompt in the one user message; content-attach, distinct from `--input`; a path of `-` names **stdin**, the portable spelling of `-f /dev/stdin`, so `bz "q1" | bz -f - "q2"` chains — opt-in, so a bare positional prompt still never reads stdin; missing/unreadable/non-UTF-8 ⇒ 66; refused with a piped canonical request or `--raw`, §5.5) · `--input <file>` (read the request from a file instead of stdin — file and pipe die at `open()`, identical path, §5.5; `--input -` is **not** special-cased; missing/unreadable ⇒ 66) · `--config <file>` (use this config file instead of the search path; only changes *which* file the config layer reads — a direct flag still beats it, §6.3).

**Control short-circuit flags:** `--login` (+ `--browser`), `--list-models`, `--list-providers`, `--count-tokens`, `--serve` (the ingress accept loop, ingress.md §7 — runs until SIGINT/SIGTERM), `--dump-config`, `--help`/`-h`, `--skill`, `--version`/`-V` — each replaces the data-plane run with a control action and exits (§5.10.1).
...
```rust
#[derive(Default, Clone, Debug, PartialEq)]   // custom Deserialize lives in config/partial_de.rs (the [[provider]] array ⇄ row-list seam)
pub struct PartialConfig {
    ...
    pub timeout:          Option<u64>,          // the silence budget, WHOLE SECONDS (config §4.3, §13.15): one value fanned onto connect / response-headers / inter-chunk-idle at the transport seam; NOT a wall-clock total
    ...
    pub providers:        Vec<(String, PartialProvider)>,     // merged sparsely per name; DECLARATION order = routing priority (§4.3.1)
...
| **78** | `EX_CONFIG` | `Config` | no provider resolved, bad/contradictory config, unknown provider/model |
...
lib (brazen) — src/
  ...
  run/
    ...
    count/
      mod.rs          `--count-tokens` (+ CountIo): read the request like the data plane, resolve, ONE count round-trip, emit input_tokens (§5.10.1)
      fetch.rs        fetch_count — the count round-trip (select_model READ-only, fill/preamble/strip, Protocol::count_tokens body)
    masq/
      mod.rs          the masquerade shell (ingress §5, §7–§11): the per-request decode_request → generate → encode_response bridge both front doors share
      listen.rs       `bz --serve` — the thread-per-connection accept loop over the injected Bind/Listener seams (ingress §7, §8)
      http.rs         hand-rolled minimal HTTP/1.1: request line + headers + Content-Length body in; Content-Length aggregate or chunked SSE out (ingress §7)
      filter.rs       `bz --in DIALECT` — the one-shot ingress filter: one dialect request on stdin, the dialect response on stdout (ingress §11)
    models/
      mod.rs          the `--list-models` control op (+ ListIo): flag parse → resolve → print; the cache's WHOLESALE writer
  ...
  canonical/
    request.rs        CanonicalRequest (model: empty==absent), Message, Content, Tool, ToolChoice, ImageSource, DocumentSource, Role
  ...
  config/
    ...
    partial/
      ...
      ingress.rs      the sparse [ingress] table (ingress §6): the masquerade's one new config surface, a top-level sibling of [[provider]], deny_unknown_fields
    partial_de.rs     custom Deserialize: the [[provider]] array-of-tables ⇄ row-list seam (§2.2)
    resolve/
      ...
      ingress.rs      lift the merged [ingress] table into IngressConfig for the --serve/--in paths (ingress §6, §7); runs only when a masquerade path asks
    ...
    errors.rs         ConfigError set (78): NoProvider / UnknownProvider / IncompleteProvider / BadValue
  ...
  protocol/
    ...
    anthropic/        mod.rs + encode/{mod,blocks,cache,count}.rs + decode/{mod,blocks,errors}.rs   (cache = auto cache_control §2.10; count = count_tokens body)
    ...
    google_cloudcode/ mod.rs + envelope.rs: the google_genai body/chunk fold in the `{model, request}` / `{response}` envelope of the Cloud Code (Antigravity) backend — providers §4.10, auth §11
  ...
  ingress/            the input-edge mirror of the egress protocol adapters (ingress.md §2): a CLIENT dialect
    ...
    mod.rs            the closed IngressId dialect enum + IngressError + decode_request/encode_response dispatch
    state.rs          IngressState — the encode-side state threaded across events (the mirror of DecodeState)
    reinject.rs       the decode-side replay-stash join: prior assistant turns recall their stashed opaque payloads (ingress §5)
    openai_chat/      mod.rs + decode.rs + encode.rs + chunks.rs + messages.rs             (OpenAI chat/completions dialect, wave 1)
  ...
  store/
    mod.rs            trait CredStore, Cred, Secret; trait ModelCache; trait Clock; trait ReplayStash
    ambient.rs        ambient discovery as data: AmbientSpec/AmbientFormat + the pure parse_ambient (auth §5.5)
    replay.rs         the fail-open ReplayStash (ingress §5): opaque reasoning-replay payloads a masquerade client's dialect cannot carry, parked by key
  os/
    browser.rs        browser_argv(os) -> argv  (the one cfg/OS-match)
  testing/            in-lib test doubles (`#[cfg(test)]`): clock.rs / store.rs / cache.rs / transport.rs / login.rs / listen.rs (the Bind/Listener MemConn double for --serve)
...
3. **OAuth — operator-configured, with ONE shipped row.** OAuth `client_id`/scope/endpoints are operator-supplied data on the auth row; the core compiles in no vendor login policy and never branches on a vendor, which is what this item is about and it is unchanged. What changed (operator ruling 2026-08-16, superseding "no built-in OAuth row ships for v0.1"): the table ships **one** `auth = "oauth2"` row, `openai-chatgpt` (auth §10.5), because a binary whose only reachable sign-in is hand-authoring an api key into a config file has no browser login at all — the capability existed and nothing on a default install could reach it. It is still pure data, still severable (delete the row, delete the capability), and still last-and-prefix-less so it moves no routing. **Anthropic subscription OAuth remains unshipped** — that vendor blocks third-party use of its OAuth tokens, which was always this item's actual reason (§4.2, §7, auth §10.5).
...
14. **`--raw` is directional (`--raw=in`/`--raw=out`, bare `--raw` = both); the split was pulled forward (owner ruling 2026-07-08, RESOLVED).** `--raw` skips both translators; its two halves — verbatim request in, verbatim response out — toggle independently. The split was originally deferred at 0.1.0 as the **one CLI change here that is not a one-way door**: bare `--raw` means "both" today and forever, so `--raw=in`/`--raw=out` is a pure, backward-compatible extension — no existing `--raw` invocation changes meaning. The owner reopened and shipped it ("sure, I can't think of a strong reason not to do this") for the compelling **normalized-in / raw-out** case: `bz`'s request ergonomics (positional prompt, config-merged model/system/params, the model cache, auth) with the **exact provider wire bytes** out — previously impossible (raw-out forced hand-writing the whole provider-native request on stdin), and the only encode-observability window since there is deliberately no `--debug` flag (§2). Implementation: the OUTPUT axis is `OutMode == Raw` (the `RawSink`, set by `--raw`/`--raw=both`/`--raw=out`), the INPUT axis is `raw_in` (send the stdin body verbatim, set explicitly by `--raw=in`/`--raw=out` and derived `= OutMode == Raw` for bare `--raw`); the two request halves (`send_encoded`/`send_raw`) and two response halves (canonical `pump` / `stream_raw`) meet at the `drive` seam, so the four combinations are two independent 2-way choices, not four pipelines. Backward-compat holds because bare-`--raw`'s input rawness *derives* from the final `OutMode`, so `--raw --json` stays normalized-json while `--raw=in --json` keeps its input rawness (§5.4, §5.10.2, §5.10.3).
15. **The three timeout knobs collapse to one — `--timeout` (one-way-door review, RESOLVED, owner ruling 2026-07-08).** The pre-0.1.0 surface had **three** per-request transport timeouts — `--timeout-connect` / `--timeout-response` / `--timeout-idle` (env `BRAZEN_TIMEOUT_*`, config `timeout_connect`/`timeout_response`/`timeout_idle`, defaults 30/120/300). They ship in released 0.0.1/0.0.2 (a breaking removal, recorded in CHANGELOG `[Unreleased]`). **Owner reasoning:** the three are *one fact* — "if it's not sending, it's not sending"; each separate knob is "a shackle of forward compatibility." They already collapse at the only *observable* layer — every one surfaces as `ErrorKind::Transport` → exit **69** (the split's sole residue is ureq's phase-named diagnostic string, which survives the collapse for free). So the surface is one `--timeout <s>` (env `BRAZEN_TIMEOUT`, config `timeout`, one default **120** in `data/defaults.toml` — tunable data, one line, §3.1). **Naming:** bare `--timeout` was chosen over `--timeout-silence`; the silence semantics are pinned in §5.10.3 (no progress for `s` seconds, per phase — connect / awaiting-headers / between-chunks) so the name cannot drift toward wall-clock. A **wall-clock total** timeout stays separately **rejected** (a footgun — it would truncate a long-but-live generation; the harness kills the child if a hard ceiling is ever wanted). **Mechanism:** the one resolved value fans onto ureq's `timeout_connect` + `timeout_recv_response` + the off-thread inter-chunk `IdleChunkReader` budget — the research's cheap option, no new mechanism, no watchdog. The fan-out lives in the pure `ResolvedConfig::timeouts()` query (which still yields the seam's `Timeouts { connect, response, idle }`, all three = the one value), so it is observable at the `WireRequest` seam and held at 100% coverage; `HttpTransport` and the OAuth-refresh inheritance are untouched. **Accepted behavior deltas** vs the old 30/120/300: (1) a silent connect black-hole now waits 120s instead of 30s before erroring; (2) one value serves both the connect and the inter-token timescales (a legitimate token gap between the old 120 and 300 would newly trip) — both owner-endorsed by "if it's not sending, it's not sending." OS fast-fails (NXDOMAIN, ECONNREFUSED) are unchanged (§5.10.3, config §4.3).
