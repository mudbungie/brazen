...
**Control short-circuit flags:** `--login` (+ `--browser`), `--list-models`, `--count-tokens`, `--serve` (the ingress accept loop, ingress.md §7 — runs until SIGINT/SIGTERM), `--dump-config`, `--help`/`-h`, `--version`/`-V` — each replaces the data-plane run with a control action and exits (§5.10.1).
