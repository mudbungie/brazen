...
- **Disambiguation is plain getopt — the caller's standard escapes, not a parser heuristic.** A token is a control operation only by being a literal `--login` / `--list-models` in the option region; everything else is a prompt, and once the prompt region begins a `-`/`--` inside it is inert text, not an option. Two standard conventions close every ambiguity and leave the choice with the caller: **(1)** a bare `--` terminates option parsing — after it every token is positional, so `bz -- --login` is the prompt `--login` (the prompt is one positional, so multi-word content is one quoted argument: `bz -- '<content>'`); **(2)** a flag value that begins with `-` uses the `--key=value` form (`--system=--login`), one token, never read as a flag. So a process wiring **arbitrary or untrusted content** as the prompt sanitizes with `bz -- '<content>'` — the same idiom as `bl create -- "$TITLE"` — and the `--` guarantees the content can never be interpreted as a flag or control op. No bare word and no injected content is ever mistaken for control surface; the §5.10.1 namespace rule holds without the parser sniffing anything.
