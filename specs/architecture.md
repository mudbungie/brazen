...
**Input-axis flag** (orthogonal to `OutMode` — does **not** set the output mode, so it composes with the projections above; §5.10.2): `--raw=in` — verbatim provider request in, canonical events out under whatever `--text`/`--json` is set.
