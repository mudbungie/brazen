...
**With NO model and NO provider, resolution defaults to the first row.** The zero-config `bz "q"` (no `--provider`, no `--model`/`BRAZEN_MODEL`) is *not* `NoProvider`: routing falls back to the **first provider row** — the table is keyed by name, so "first" is the lexicographically-first provider name — and the empty model seed takes `select_model`'s **first cached model**. Symmetric defaults: "no specification" → (first provider, first cached model), the empty-input dissolve of the routing special case. This is distinct from `bz -m opus "q"` above — a *given* model owned by no row still cannot pick a provider, because guessing one for a named model needs the forbidden vendor table; with *no* model there is nothing to guess against, so the first row is the honest default. The lone residue of `NoProvider` on the no-model path is an **empty** provider table (nothing to default to). **`--login` is the exception** — a credential write must name its target, so it requires an explicit `--provider`/configured `provider` and never takes this default ([auth.md](auth.md) §7.1).
