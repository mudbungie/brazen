...
- **`--list-providers`** is the read of the **effective** provider table (config.md §6.1, bl-b679) — the local sibling of `--list-models`, which lists one row's *models*. It resolves **no** provider and routes to none: it folds the config the way a run does (`flags.or(env).or(file).or(defaults())`) and completes **every** row through the one `row::complete` lift, printing `name` / `protocol` / `auth` / `credential` in `providers` order, so the listing's order **is** routing priority and its head **is** the zero-config default. It is the one listing that is **offline by construction** — it takes stdout/stderr/`CredStore` and no `Transport` at all, so zero round-trips is a property of its type. It exists because `--dump-config` deliberately drops the defaults operand (§6.2, config.md §6) and therefore can never show the embedded rows; `--list-providers` keeps that operand because it writes no file. `credential` is `resolved_secret`'s answer minus the network (`not required` / `inline` / `stored` / `ambient` / `missing`), so it cannot disagree with what a run would do.
