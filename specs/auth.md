...
fn resolved_secret(store: &dyn CredStore, auth: &AuthCtx) -> Result<Secret, Error> {
    ...
    match store.get(auth.store_key) {
        ...
        Some(Cred::OAuth2 { .. })    => Err(auth_error(&format!("stored credential for provider `{row}` is OAuth2 …"))), // row says api_key/bearer but stored cred is OAuth — config drift, 77
        None                         => Err(auth_error(&format!("no credential for provider `{row}`: set BRAZEN_API_KEY or run `bz --login --provider {row}`"))), // → 77
    }
}
```
...
pub enum AmbientFormat { ClaudeCode, Codex, ApiKeyEnv }
```

- **`path` is operator DATA — the source LOCATOR the format reads**: a `~`/`$HOME`-expanded filesystem path for a file format, or an environment-variable NAME for `api_key_env`. **`format` selects the pure parser** that maps that source's bytes to a brazen `Cred`. Both pieces explicit, neither in core code — deleting the `ambient` line deletes the capability with no code edit (severability, architecture.md §1).
- **`AmbientFormat` is a closed enum, not a JSON-pointer DSL.** Each foreign shape needs a parser anyway; one variant per known source is less mechanism than a speculative mapping language (architecture.md "build less"). A second source is a second variant + parser — `ApiKeyEnv` (below) is exactly that.

**`api_key_env` — the vendor-conventional key alias as a row-scoped, store-miss source (bl-5a43).** A vendor's customary env var (`ANTHROPIC_API_KEY`) is *also* a credential another tool on the box already holds, so it is a second ambient source, not a config-projection field. The built-in anthropic row names it as data:
...
**`codex` — the Codex CLI's ChatGPT sign-in, on the row that already speaks to it (bl-ee04).** The shipped `openai-chatgpt` row (§10.5) names it as data:

```toml
ambient = { format = "codex", path = "~/.codex/auth.json" }   # on the oauth2 `openai-chatgpt` row
```

The file is that tool's own store: `tokens.{access_token, refresh_token, account_id}`, plus a sibling `last_refresh`. `parse_ambient(Codex, bytes)` maps it to a `Cred::OAuth2` — the account id **read from the file's field**, not re-derived from an id_token (§10.4's fact is already extracted there), `scope` `None` (the file records none), and `expires_at` from the access token's own **`exp`** claim through the same `jwt_exp` §10.3 uses for this vendor, whose token endpoint returns no `expires_in`. `exp` is already absolute, so nothing adds `now`.

- **An access token with no readable `exp` is `None`** — the no-creds path, like every other missing field. `last_refresh` cannot stand in: it records when the token was **minted**, so turning it into an expiry needs a token lifetime the vendor states nowhere in this file and brazen would be **inventing**. A `Cred` carries an absolute instant or it is not a `Cred` (§5.1).
- **`jwt_exp` moved to the crate root** with this format. It now has two readers on opposite sides of the credential seam — `auth::oauth` reading brazen's own token response, and this parser reading a foreign tool's stored token — and a pure byte→field reader belongs to neither owner.
- **Read-only, like every borrowed source.** brazen never writes `~/.codex/auth.json`, never refreshes against it, and never copies it into its own store. The Codex CLI's own refreshes are picked up simply by the next `bz` re-reading the file.

**The cred-fetch is one provenance-carrying query; ambient is its empty case.** Both `apply` impls resolve through one helper: a store hit yields `(Owned, cred)`, otherwise the row's optional discovery yields `(Borrowed, cred)`. This preserves the source fact at the point that knows it instead of reconstructing it later from the credential shape (both sources return the same `Cred`). A row with no `ambient` block makes the borrowed arm `None`: the general path with an empty input, not a special case (architecture.md §1). `AuthCtx` still needs only `ambient: Option<&AmbientSpec>`; provenance is the fetch result, not persistent config or a second field on `Cred`. **The borrowed arm carries the LOCATOR it read** — `CredSource::Borrowed(String)`, not a bare discriminant — because every message about borrowed state has to name the file the operator can act on ("refresh it with the tool that owns it" is not something anyone can look up), and reconstructing the path later from `AuthCtx.ambient` would be a second home for a fact the fetch already knew ("carry the fact", architecture.md).
...
**The row's sources are a LIST read in order, not a single verdict (bl-ee04).** The store is looked at first and the ambient block second — on a store **miss**, and equally on a store hit this run **could not make fresh**. A refresh the authorization server refuses is permanent for *that* credential (no retry dislodges it, §6.2) but says nothing about a tool on this box that is signed in right now, so refusing there while a usable credential sits beside us is a dead end the operator cannot exit without deleting a file by hand. `apply` therefore walks: fresh store hit → use it; stale store hit → refresh it; **refused refresh → the ambient source, exactly as a miss would have** → a fresh borrowed credential answers, a stale one does not (borrowed is read-only either way). Only when no source answers is it an error, and then the message names the ambient file rather than ordering a `bz --login` the operator may not need.
...
impl Auth for OAuth2Auth {
    fn apply(&self, wire, ctx, auth, store, clock, tx) -> Result<(), Error> {
        ...
        let Some(fetched @ FetchedCred { cred: Cred::OAuth2 { access_token, refresh_token, expires_at, scope, .. }, .. })
            ...
            else { return Err(auth_error(&format!("not logged in for provider `{row}` … run `bz --login --provider {row}`"))); };  // → 77

        let token = if is_expired(expires_at, clock.now()) {
            if let Borrowed(path) = fetched.source {
                return Err(auth_error(&format!("the ambient credential at {path} is expired; refresh it with the tool that owns that file, …")));
            }
            ...
            let fresh    = parse_token_response(&bytes, clock.now())      // PURE; sets ABSOLUTE expires_at; Err = AuthError::Fatal
                              .or_else(|_| return spent(store, auth, clock))?;  // ANY unparseable response (status-blind): the ambient source, else 77 (§6.2)
...
**Every auth refusal NAMES the row (bl-e809).** "this provider" names none of them, and the gap is widest where it costs most: under a `--serve` masquerade the row was chosen by an inbound model string or a `--provider` pin the CLIENT never saw, so a 401 reading "no credential for this provider" is the one sentence an operator cannot act on. The name is `AuthCtx.store_key` — the resolved row name (§1.3), already the single data-plane home for it — and it is also the argument `bz --login --provider` wants, so the hint that follows is copy-pasteable rather than a `<id>` placeholder the reader must still resolve. This adds no field and no lookup: the error site already holds the `AuthCtx`.
...
- **Borrowed means read-only, including brazen's own store.** An expired ambient OAuth credential returns 77 before `Transport::send` and before `CredStore::put`; a fresh one is used only for the current request. The error directs the operator to the owning tool **by naming its file** (the path rides `CredSource::Borrowed`, §5.5), not to `bz --login`, because brazen does not own that login. Static borrowed API keys/bearers have no staleness operation and are unchanged.
...
- **A failed refresh is exit 77 when no other source answers — single path, status-blind, non-alarming.** **Any** refresh response the token parser cannot read is a refusal of *that* credential and hands off to the source walk above (§6): with a row-named ambient block, a **fresh** discovered credential answers the request instead, and only a source that cannot answer becomes an error — whose message then names the ambient file, because "re-run `bz --login`" is the wrong instruction for a box whose other tool holds a working sign-in. With no ambient block, or none that can answer, it is one `RefreshFailed` → **exit 77**, *regardless of HTTP status*: a permanent `invalid_grant` (revoked/expired refresh token) **and** a transient token-endpoint fault that still returns a body (a 503/429 error page) collapse to the **same** code. `apply` does **not** peek `resp.status` to fork the exit code — the operator's corrective action is identical either way, so plumbing status through the refresh seam just to split 5xx→70 / 429→69 from 77 is mechanism for a non-problem (AGENTS.md: build less). This **mirrors the device-poll path** (a failed poll is also one auth fact). Because the fault may be transient, the message is deliberately **non-alarming** — `token refresh failed for provider \`<row>\`; re-run \`bz --login --provider <row>\` if this persists` — it does **not** assert the credential is revoked/expired and does **not** order an unconditional re-login: a transient 5xx/429 is **retryable, and retry is the caller's job**, not `bz`'s (the data plane is non-interactive and does not loop). Refresh **never escalates to a browser** — blocking the data plane on interaction is forbidden (architecture.md §7.1); interaction is quarantined in `bz --login` (§7).
...
**One built-in OAuth row ships** — `openai-chatgpt` (§10.5), and only that one. This **supersedes** the original "no built-in OAuth row ships for v0.1" (operator ruling 2026-08-16). The reason the original rule failed its own goal: with no oauth2 row in the table, `bz --login` is unreachable on a default install — `resolve_oauth` refuses a resolved row that carries no `oauth` block (78), so the whole of §7 was a capability nothing out of the box could enter, and a first-time user's only path to a credential was hand-authoring an api key into a config file they first had to know existed. Shipping the row changes no code and adds no branch: it is the same operator-supplied data, supplied. What the original rule was *actually* protecting is stated exactly, and still holds — **no row ships for a vendor that restricts third-party use of its subscription tokens**, which is why Anthropic subscription OAuth is not in the table and no turnkey recipe for it is published.

**`--login` requires an explicitly named provider.** It resolves through the same `flags > env > file > defaults` fold as a run, but — unlike the data plane — it does **not** inherit the zero-config first-provider default (architecture.md §4.3): `bz --login` with no `--provider` and no configured `provider` is `NoProvider` (78), not a silent login to the first row. The asymmetry is deliberate: login **writes** a credential under a provider key, and the target of a write must be chosen, never guessed — auto-picking the first-declared row would persist a token against an unintended (and usually non-OAuth) provider. `resolve_oauth` enforces this by refusing an absent `provider` *selector* (distinct from the data plane's zero-config first-row default, `providers.first()`) before resolution would apply the default; the resolved row must then carry an `oauth` block (else also 78).
...
ambient    = { format = "codex", path = "~/.codex/auth.json" }  # §5.5 — borrow the Codex CLI's own sign-in on a store miss, read-only
...
[provider.models]                                             # the --list-models discovery override (model-discovery §3.2); openai_responses owns only the DEFAULT shape, Codex diverges
path      = "/models"                                         # same path as the protocol default — pinned for explicitness
query     = [["client_version", "0.0.0"]]                     # Codex /models 400s without it ({'loc':('query','client_version'),'msg':'Field required'}); 0.0.0 = the full catalog (see below)
array_key = "models"                                          # Codex returns {"models":[…]}, NOT the protocol-default {"data":[…]}
id_key    = "slug"                                            # each entry's id is `slug`, NOT the protocol-default `id`
```
...
**The Codex backend rejects more than `max_output_tokens` — `temperature` and `top_p` 400 the same way** (validated live 2026-06-17, bl-d54a): the second and third fields that lift bl-73d8's deferral of a per-row strip. Omitting `max_tokens` from `body_defaults` stops the *default* from carrying it, but an operator passing `--max-tokens`/`--temperature`/`--top-p` (or those keys in the input JSON) would still 400. `unsupported_body_keys = ["max_tokens","temperature","top_p"]` (above) closes that hole: `strip_unsupported` drops each from the canonical request after `fill_absent`, so the value is silently normalized away and the request streams (config §4.1.1 — the inverse of `body_defaults`). With it, every Codex *request-body* quirk is handled by data, none by operator discipline. One quirk remains, on the *discovery* path — and it gets the same treatment:

**Why `[provider.models]`, and the `client_version` pin (model-discovery §3.2).** Codex's `/models` route is NOT the protocol default: it 400s without a `?client_version=X.Y.Z` query (`[{'type':'missing','loc':('query','client_version'),'msg':'Field required'}]`) and returns `{"models":[{"slug":…}]}`, not the OpenAI-standard `{"data":[{"id":…}]}` — so `bz --list-models --provider openai-chatgpt` without this block 400s, the discovery analogue of the body quirks above. The `[provider.models]` block carries all three facts as DATA — the query the route demands plus the `models`/`slug` decode keys — over the protocol default (the SAME per-row override mechanism bl-eaf9 built; `openai_responses` owns only the *default* shape because it ALSO fronts standard api-key OpenAI, which keeps `data`/`id` and needs no query). **Why `client_version = "0.0.0"`:** the list is server-gated by version, so a list verb must pin one that returns the *whole* catalog, not one real client's slice. Validated live 2026-06-29: `0.0.0` (and `1.0.0`/`99.0.0`) → the full 7 models; a real `0.x` build gets gated — `0.36.0` → empty `{"models":[]}`, `0.99.0` → a 3-model subset. `0.0.0` is the sub-release sentinel (below every real codex-CLI release, so it can never be retro-gated to a subset the way a real-looking version could) that the backend treats as ungated. A pin can still go stale → a valid empty list → exit 0 with a one-line stderr note (§3.2), re-pin then; never a silent failure. So now **every** Codex quirk — request-body and discovery alike — is handled by data, none by operator discipline.
...
**And with no `bz --login` at all, when the Codex CLI is signed in on the box (bl-ee04).** The `ambient` line above makes that tool's `~/.codex/auth.json` the row's store-miss source (§5.5), so `bz --provider openai-chatgpt -m gpt-5 "hi"` answers on a fresh install. It is one line of the same row data — no code, no vendor branch, severable with the rest of the row — and it is the *second* reason this row earns its place in the table: the login it makes reachable, and the login it makes unnecessary.

> **DECIDED — this row ships built-in (operator ruling 2026-08-16, bl-77fa).** The question was flagged here and deferred: ship the row, or keep it a recipe the operator pastes? The recommendation was *recipe*, to preserve §7's "the core never ships vendor OAuth policy". The ruling went the other way, and the argument that settled it is the one the deferral did not weigh: **with no oauth2 row in the table, `bz --login` is unreachable — the binary offers no browser sign-in at all**, and a stranger's only route to a credential is authoring an api key into a config file. A capability nothing on a default install can enter is not a preserved principle, it is a missing feature.
>
> The principle survives intact, because shipping the row **is not shipping policy into the core**: this block is data (`data/defaults.toml`), read by the same vendor-blind `OAuth2` impl off `AuthCtx`, and the row is severable — delete it and the capability goes, with no Rust touched. Three constraints came with the ruling and are pinned by test (`src/tests/oauth2_provider_recipe.rs`):
>
> - **Exactly one** oauth2 row ships. Each one is a vendor whose login policy the binary then carries, so the count is the budget.
> - It is **last** in the table and claims **no `model_prefixes`**, so nothing about routing or the zero-config head row (`anthropic`) moves; it is reached only by explicit `--provider`, like `openai-responses`.
> - The `client_id` is Codex's **published public** client and the flow was live-validated end to end (§10.7) — the two facts that make it legitimate to ship rather than merely convenient.
>
> **Anthropic subscription OAuth is still NOT shipped** and this ruling does not touch it: that vendor restricts third-party use of its subscription tokens, so neither a row nor a turnkey recipe is published (bl-a661). The general mechanism above stays available to an operator who may use it.
...
### 10.7 Live-flow validation results (validated 2026-06-16 / re-confirmed 2026-06-17 / re-probed 2026-07-31)
...
- **Codex backend mandatory request fields** — each omission is a **400 with a descriptive `detail`** (literal service messages, captured live). The set is **not static**: it is re-probed with `--raw` (which bypasses `encode`, so neither the row's `body_defaults` nor `serve`'s stream force can mask a mandate), and one member has since lapsed:
  - ~~missing/empty `instructions`~~ — **NO LONGER A MANDATE as of 2026-07-31 (bl-30b0).** It 400'd `{"detail":"Instructions are required"}` through 2026-06-17; it now **200s**. Verified twice: canonically (`bz --json` with no `system` → exit 0 and a normal completion) and on the raw wire (no `instructions` key → `response.created` echoing `"instructions":null`). brazen never forced the field — `encode` omits it when `system` is absent/empty (§3.2) — so nothing in brazen changes; the live tripwire moved from the fuzz suite's error matrix to its **acceptance** set, where it now asserts the omission is accepted and would fail loudly on a silent re-imposition.
  - `stream` not `true` → `{"detail":"Stream must be set to true"}` (**still in force**, re-probed 2026-07-31)
  - `store` not explicitly `false` (brazen carries `store` via the `extra` flatten passthrough, §3.2) → `{"detail":"Store must be set to false"}` (**still in force**, re-probed 2026-07-31)
