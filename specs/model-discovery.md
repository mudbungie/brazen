...
bz --list-models                                # provider from `--provider`/configured `provider`/model; else the FIRST provider row
```

- **Provider resolution is the SAME query** (config.md §7): an explicit `--provider`, else the row that owns a configured `model`, else (nothing specified) the **first provider row** — discovery shares the data plane's zero-config default (architecture.md §4.3). So a bare `bz --list-models` lists the default provider's models. No model is *needed* (the flag lists them), so a bare `--provider` is the common form; `NoProvider` (78) is left only for an empty provider table.
