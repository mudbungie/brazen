...
**The cost model (architecture.md §1, amended).** A **fully-specified** model (a full wire id its row owns by `model_prefixes`, or an exact `model_aliases` key) stays **one round-trip** — resolution is offline, unchanged. An **imprecise** model (a partial, or absent) prepends **exactly one** model-list probe to the *single resolved provider*, then the generation round-trip. This is the bounded, explicit-by-imprecision price of not naming a full id — **not** agentic behavior (no loop, retry, or fan-out — §2), and **not** caching (the list is fetched fresh and discarded — architecture.md §2's no-state non-goal holds; nothing is written to disk).
...
- **Exit codes** (architecture.md §8): `0` success; `78` provider unresolved / empty list; `77` auth; a non-2xx models response maps through `ErrorKind::from_http_status` (4xx→69, 5xx→70) like the data plane; a malformed body is `ErrorKind::Provider`.
...
> **A model needs a probe iff it is absent (`""`), OR it is neither an exact `model_aliases` key nor owned by any of the resolved row's `model_prefixes`.** (i.e. `!row_owns(resolved_row, routing_model)`, the §7 predicate, scoped to the *already-resolved* row — extended to the explicit-`--provider` case, which §7 routing does not check today.)
...
- `probe == false` → `cfg.model` is the final wire id (a prefix-owned id passed verbatim, or an alias substituted to one). **No probe; one round-trip — unchanged.**
...
| `needs_probe` (resolution) | A prefix-owned full id and an exact alias → `false`; a partial and an absent model → `true`; computed for both the explicit-`--provider` and the routed cases. |
...
| `list-models` verb | Run-level with a `MockTransport` models body: `--json` emits the `{"models":[…]}` object; default mode emits ids one-per-line with ` (default)` on the default; `NoProvider`/auth/non-2xx map to 78/77/69-70 on stderr. |
