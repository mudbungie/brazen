...
### 5.5 The window on the wire — the row's fact, carried in-band

A harness that meters context fullness needs the **denominator** for the usage counters it already records: the model's context window. It is a row fact (§3, `Model.context_window`), so the read surface (§2, `--list-models --json`) already serves it — but a consumer that keeps no model table and runs no list call (litany, its ARCH §4.2) would need a second call *just* to divide by it. So the same fact rides the **event stream it already consumes**: the resolved row's `context_window` is stamped onto every `Event::Usage` of the turn (canonical-protocol §3.2).

- **The lookup is free.** `serve` already reads the cache to resolve the seed (§5.2) — the window is read off the SAME resolved row, in the same local read, on the way past. No GET, no second seam.
- **One stamp site.** No provider serves the window on a generation response, so every decoder leaves the field `None` and `run::drive::canonical_events` — the one home of "response half → canonical events" — carries it in. This is the carry-the-fact rule (AGENTS.md) that already governs the §5.3 provenance hint and the `Retry-After` stamp: a fact the body never held, threaded from the component that knows it, never re-derived downstream from a proxy.
- **Absent stays absent.** A row whose provider serves no limit (§3: OpenAI/Ollama today, Anthropic's limits), an id `select_model` could only pass through `Verbatim` (the window belongs to the ROW, not the provider), and the `--raw` path (which never consults the cache) all leave the key **off the wire entirely** — never a fabricated number, and byte-identical to the pre-window event.
- **The window a row states is the window reported.** Learning (§5.4) appends a bare id with no metadata, so a learned row reports no window until `--list-models` REPLACES the list with the served facts. Discovery is authoritative when it runs; nothing invents in its absence.
...
| the window in-band (§5.5) | A cached row STATING a window → every `usage` event of the turn carries `"context_window":n` and no other event grows the key; the SAME row stating none → the key is absent from the whole stream (byte-identical to the pre-window shape); a seed the cache resolves `Verbatim` against a window-bearing list → no key (the window is the row's, not the provider's). `MockTransport` + `MemoryModelCache`, end to end through `run`. |
