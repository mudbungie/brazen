...
### 5.5 The window on the wire — carried in-band, from the three places it can come from

A harness that meters context fullness needs the **denominator** for the usage counters it already records: the context window the turn runs in. A consumer that keeps no model table and runs no list call (litany, its ARCH §4.2) would need a second call *just* to divide by it — so the number rides the **event stream it already consumes**, stamped onto every `Event::Usage` of the turn (canonical-protocol §3.2).

**Three sources, one ladder, read most-specific-first.** The window is not one fact with one home; it is one QUESTION — *what does this turn divide by?* — that three different things can answer, and they are ordered by how close each sits to the request:

1. **What THIS request's body pins** — `Protocol::pinned_window(&req)`, a dialect declaration like `models_shape`/`tuning`, default `None`. Ollama's `options.num_ctx` is the only shipped instance: it **truncates** the model's capacity to the size the server will actually allocate, so where it is set it is the honest denominator and the model's capacity is not. It reaches the body only through the `extra` fold — a row's `body_defaults = { options = { num_ctx = N } }` or the request's own `extra` (config §4.1, bl-f19d) — so it is read off the canonical request AFTER `fill_absent`/`strip_unsupported`, which is exactly the body `encode` is about to project; never re-parsed out of encoded bytes.
2. **What the provider's list SERVED** — `Model.context_window` on the resolved entry of the local cache (§3), read in the SAME local read `serve` already performs to resolve the seed (§5.2). No GET, no second seam.
3. **What the provider ROW DECLARES** — `context_windows` on the `[[provider]]` row (config §4.7), a `wire-model-id → limit` table. Observation beats declaration, per model: a served number moves with the provider, a declared one is whatever its operator last knew.

Rung 3 exists because rung 2 is empty on nearly every turn. Of the shipped dialects only Google's list serves a limit at all; Anthropic, OpenAI and Ollama serve none (§3.1), so before the row could state one the denominator was dark for almost every request, and each harness above brazen kept its own model table to divide by — a second representation of a fact that has to reach the request, the event, and every consumer, and which only a row-level statement reaches all three of.

- **One stamp site.** No provider serves the window on a generation response, so every decoder leaves the field `None` and `run::drive::canonical_events` — the one home of "response half → canonical events" — carries it in, whichever rung answered. This is the carry-the-fact rule (AGENTS.md) that already governs the §5.3 provenance hint and the `Retry-After` stamp: a fact the body never held, threaded from the component that knows it, never re-derived downstream from a proxy.
- **Absent stays absent.** A model no rung answers for — a provider serving no limit on a row declaring none, an id the row's table does not name, an unreadable or out-of-range pinned value, or the `--raw` path (which never consults the cache and never encodes a body) — leaves the key **off the wire entirely**: never a fabricated number, and byte-identical to the pre-window event.
- **Declaration does not enter the cache.** The row's table is read at the stamp, not folded into `Model.context_window`, so `--list-models` keeps answering what the provider SERVES and the cache stays a record of observation. Two answers to one question would otherwise become two homes for it, one of them writing the operator's guesses into a file discovery rewrites wholesale.
- **The window a source states is the window reported.** Learning (§5.4) appends a bare id with no metadata, so a learned model reports no served window until `--list-models` REPLACES the list — the row's declaration still answers for it in the meantime. Discovery is authoritative when it runs; nothing invents in its absence.

**The row syntax** (config §4.7), one entry per wire model id:
...
| the window in-band (§5.5) | The ladder, end to end through `run` on `MockTransport` + `MemoryModelCache`: a cached entry STATING a window → every `usage` event of the turn carries `"context_window":n`, all agreeing, and no other event grows the key; the SAME entry stating none → the key is absent from the whole stream (byte-identical to the pre-window shape); a seed resolved `Verbatim` against a window-bearing list → no key; a `[[provider]]` row DECLARING one for the resolved wire id → that number, and for a DIFFERENT id → no key; the same row's declaration beaten by a served window on the same model; an ollama row pinning `body_defaults = { options = { num_ctx = N } }` → N, beating a served window, and an unreadable `num_ctx` falling through to it. Plus the pure per-dialect table (`protocol_pinned_window.rs`): `pinned_window` reads `options.num_ctx` for `ollama_chat` and `None` for every other shipped dialect; absent/non-object/zero/over-`u32` state nothing. |
