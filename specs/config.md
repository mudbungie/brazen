...
A `[[provider]]` row's `body_defaults` is the **lowest-precedence operand for the request body** — what the row pins when the request, flags, env, and file all leave a field unset. It generalizes the former `default_max_tokens` (a single scalar that defaulted `max_tokens`) into **one map** so a row can also pin `store`, etc. (the OpenAI ChatGPT-SSO Codex backend mandates `store:false`, auth §10.5/§10.7; its `stream:true` mandate is satisfied automatically — `serve` forces streaming for every row, §4.2, so a `stream` pin is moot). It is resolved in two moves, mirroring the canonical request's own split into typed gen fields + the `extra` valve:
...
**But the wire value is FORCED, not folded: `serve` sets `req.stream = Some(true)` after `fill_absent`, on the canonical path only (bl-20d5, bl-9e3d).** brazen always wire-streams (architecture.md §3.2): `serve` both requests streaming **and** SSE/NDJSON-decodes the 2xx in `drive`, which has **no non-stream-2xx fold** — every concrete protocol frames a stream, and a `stream:false` body would come back as a single JSON object the framers can't cut. So an explicit `Some(false)` from request, flag, env, file, or a `body_defaults = { stream = false }` pin is **overridden, not honored** — the fill above still runs (the value is observable in `ResolvedConfig`/`--dump-config`), but the wire always carries `stream:true`. A row pinning `stream = true` (the Codex backend's mandate, auth §10.5) is therefore redundant-but-harmless; pinning `false` is moot. The one escape hatch for exact non-stream wire bytes is `--raw`, which bypasses `encode` entirely. Pure `encode` keeps `unwrap_or(false)` — at the protocol layer, absent means "don't ask"; the always-stream policy is `serve`'s, so a direct `encode` caller is unaffected.
