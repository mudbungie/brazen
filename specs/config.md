...
It also computes the **owned-vs-probe** query and stores it as `ResolvedConfig.probe: bool` (model-discovery.md §5): `probe = routing_model.is_none() || (row_has_prefixes && !aliased && !prefix_owned)`, i.e. the model is absent (need a default), **or** the row opts into fuzzy matching by declaring `model_prefixes` and the model is neither an exact alias nor owned by one of those prefixes (a partial seed to expand). A row with **no** `model_prefixes` opts OUT of fuzzy matching: a *present* model is taken LITERALLY (the final wire id, never a seed), so such a row never probes for a present model — only an absent one (which still needs a default) does. The `row_has_prefixes` guard is the fix for the regression where a prefix-less row (e.g. the shipped `openai-responses`, or a custom Codex row) fired a needless — and on Codex, fatal — list-models GET on every fully-qualified `--model`: `!prefix_owned` alone is a lossy proxy that conflates "the row does fuzzy matching and this isn't a known id" with "the row does NO fuzzy matching at all." When `probe` is `false`, `model` is the final wire id (verbatim or substituted). When `true`, `model` holds the **seed** (the partial verbatim, or `""` when absent) for `serve` to expand against a live model-list probe — the fact is *carried* because the prefixes that decide it are consumed at resolve and not retained, so it cannot be recomputed downstream (architecture.md §3.5; the carry-the-fact rule). Resolution stays pure (it has no transport); only `serve` does the probe. This does **not** widen the routing errors: a partial with no resolvable provider is still `NoProvider` (the table above), because a seed cannot select a provider.
...
| `probe` query (§7, model-discovery.md §5) | A prefix-owned full id and an exact alias → `probe == false`; on a row WITH prefixes a partial → `probe == true`; an absent model → `probe == true`; and the regression guard — a **prefix-less** row with a *present* model → `probe == false` (literal, no fuzzy expansion), with an *absent* model → `probe == true` (needs a default); for both the explicit-`--provider` and the routed cases. Pure, from literals. |
