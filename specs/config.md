...
3. **model→provider routing is a query, not a second table.** The user names a provider explicitly **or** brazen takes the **first row, in priority order, that owns** the model — its `model_aliases` spells it, or one of its `model_prefixes` claims its family. **Priority is declaration order** (§2.2): several rows may own one model, and the earliest wins — a greedy match over the operator's own list, not a silent pick (architecture.md §4.3, §4.3.1).
...
pub struct PartialConfig {
    ...
    #[serde(default)]
    pub providers:   Vec<(String, PartialProvider)>,     // sparse, name + row, in DECLARATION order = the routing priority list (§2.2); merged per-name, per-field (§3.2)
...
pub struct PartialProvider {                             // every Provider field made Option so a file can patch ONE field
    ...
    pub exec:               Option<String>,              // the subprocess program for an exec-transport dialect (claude-code.md §7.1); substitutes for base_url — a row carrying `exec` may omit `base_url` (completed as ""); unread on HTTP-dialect rows
...
### 2.2 The `[[provider]]` array ⇄ row-list seam
...
A custom `Deserialize` for `PartialConfig` reads the `[[provider]]` array **in order**, lifting each table's `name` out and storing the remainder as a `PartialProvider`. A **duplicate `name` within one file** is a `Config` error (78) — surfaced against a seen-set, never last-wins, because two rows for one name is a contradiction inside a single layer (cross-layer override is the fold's job, §3.2).

**The `provider` key is overloaded by value SHAPE, and dispatch is a visitor — never `serde(untagged)`.** A string selects a provider (`provider = "anthropic"`, §4), an array-of-tables defines rows (`[[provider]]`); one file carries only one form, so the two never collide. The custom `Deserialize` picks the arm from the value's shape (`visit_str` ⇄ selector, `visit_seq` ⇄ rows) rather than trying both and keeping whichever parses. `untagged` buffers the value into serde's private `Content` and **discards the losing arm's error**: with it, one mistyped row key made the whole file read `data did not match any variant of untagged enum ProviderField` pointed at line 1 column 1, so §2.3's deny fired but its message — the offending key, its line, the valid keys — never reached the operator, and the same opaque text was what an older `bz` emitted for a *newer* row key it simply did not model yet (bl-50af). The shape dispatch has no buffer and no losing arm: the row's own error propagates verbatim, with its span. This is the same ruling `Tool` records at architecture.md §3.1 — untagged gives unusable errors — and it removes serde's private re-encoding from the config parse path, so the parse is not hostage to a dependency bump under a lockfile-free `cargo install`.

`Serialize` (for `--dump-config`, §6) runs the inverse: rows emit in list order with `name` re-injected, so a dumped config re-parses to the **same priority** — the encoding round-trips the order because the order never left the list. **Priority has exactly one wire form: the `[[provider]]` array's own line order**, which is what the operator already reads and writes. There is deliberately **no** `provider_order`-style config key — a second wire form for one fact, and a way to author an order naming rows that do not exist. (§6 decision 4 owns the emission rule and what it supersedes.)

**Distinct row names are distinct credential files — the multi-account / rotation seam.** Because the cred-store key is the row name (the file is `<name>.json`; `store_key = provider.name`, auth.md §1.3, §5.2), N rows with distinct names give N **independent** credential files. So `anthropic` and `anthropic-work` are separate accounts, keys, and rotation targets for one vendor — a **supported interface guarantee** stated in full at auth.md §5.2, not an accident of this row-list encoding. Give only **one** of them `model_prefixes`/`model_aliases` and the account-scoped row is reached the honest way, with an explicit `--provider <name>` (architecture.md §4.3). If both claim the family, this is not an error — greedy-first silently routes to whichever is declared earlier (§7 step 3) — which is exactly why an account-scoped row should claim nothing: you want `--provider` to be the only way in, not a race with your own line order.
...
impl PartialConfig {
    ...
    fn or(self, other: PartialConfig) -> PartialConfig {
        PartialConfig {
            ...
            providers:   merge_providers(self.providers, other.providers),  // §3.2: hi's rows first, per-name field merge
...
{
    // hi's rows in hi's order — each field-merged (`PartialProvider::or`) with lo's row of
    // the same name if it has one — then lo's rows whose names hi never mentioned, in lo's
...
It overrides that one field — and it also **moves the row**, which is the one sharp edge of per-field merge meeting priority. `providers.first()` after the merge is hi's first row when hi has any, else lo's, so this subsumes the old `default_provider: self.default_provider.or(other.default_provider)` **exactly** — the identical result, now read off the head of a list instead of stored as a fact of its own. And the law survives: dedup-keep-first over concatenation is grouping-independent, so the merge is **associative** and §3.1's "add a layer = add an operand" still holds.

**But a redeclared row takes YOUR position while keeping the defaults' claims**, and that deserves stating outright. A user row `[[provider]] name = "openai"` carrying only a `body_defaults` tweak still resolves with the **embedded** row's `model_prefixes = ["gpt-", …]` — fields fall through — while taking the **user file's** position. So *touching one field of a defaults row is also a priority claim for everything that row owns*: put such a tweak above a `gpt-4o` alias row and it silently re-arms `gpt-` ahead of the alias, and the masquerade of ingress.md §6 stops working with no diagnostic. Declare the row that owns a model **before** any row you redeclare, or reach the redeclared row by explicit `--provider`.

**There is no replace-semantics, and there is no tombstone (bl-d67a).** A user file that declares its own `[[provider]]` rows does **not** hide the embedded ones: every default row whose name the user never mentions survives the merge, in `defaults.toml` order, behind the user's rows. So a row shipped in a later brazen (`claude-code`, bl-b0b6) reaches an operator who already has a config file, with no edit on their part — which is the whole point of a floor. Mirroring a defaults row into a user file to "make it visible" is therefore never necessary and is exactly the two-homes-for-one-fact drift the fold exists to prevent. The reported symptom that motivated this ruling was two *other* facts wearing replace-semantics as a costume: (1) a **stale installed binary** — a `bz` older than the row cannot resolve a row it does not embed, and `bz --version` is the check; (2) `--dump-config` **omitting the defaults operand by design** (§6), so no default row ever appears in a dump, with or without user rows. Neither is a merge fact. Correspondingly there is **no** tombstone syntax for deleting a default row: a row you never name costs nothing (routing is a `find` over ≤10 rows), a row whose claims you dislike is disarmed by redeclaring it with empty `model_prefixes`/`model_aliases`, and a delete verb would be a second, subtractive merge algorithm beside the one `or`.
...
| `BRAZEN_TIER` | `service_tier` (parsed `priority\|standard` via `FromStr`; unparseable → §7 `Config`) |
| `BRAZEN_IMAGE` | `image` (parsed bool; unparseable → §7 `Config`) |
...
- **Every other key is request passthrough**: it merges into `cfg.extra` (the row's keys winning over the top-level `extra`, being more specific), and `fill_absent` seeds it into `req.extra` beneath the request's own keys. It reaches the wire through the **same `req.extra` fold every encoder already runs** (`protocol::json::fold_extra`: typed fields win, and two objects merge one level so a nested passthrough key survives beside the typed ones — openai-chat-mapping.md §2.1.1, bl-f19d) — no encoder change, and the live seam, not the formerly-dead `ProviderCtx.extra` (§9).

**`image` follows `service_tier` exactly** (providers.md §6.3, bl-842b) — `--image`, `BRAZEN_IMAGE`, top-level `image = true`; the same `or` fold and `fill_absent` fill; not taken from `body_defaults` (a raw `generationConfig.responseModalities` or `image_generation` tool pinned there rides `extra` verbatim, and the typed knob composes with it).

**`service_tier` follows `reasoning` exactly** — the same rungs (`--tier`, `BRAZEN_TIER`, top-level `service_tier = "priority"`), the same `PartialConfig::or` fold, the same `fill_absent` fill, and the same deliberate absence from `into_resolved`'s `take_*` list: a `body_defaults.service_tier` stays raw passthrough so a vendor lane the two-rung enum cannot spell (OpenAI `flex`/`scale`) rides `extra` to the wire, where the typed knob wins on the same key (providers.md §6.2). Everything the next paragraph says about `reasoning` reads the same way for it.
...
**Full precedence for any body field: request (typed field / `extra` key) > flag > env > config file > row `body_defaults` > the encoder's protocol baseline.** It holds **per key, one level into an object too**: a row's `body_defaults = { options = { num_ctx = 32768 } }` composes with the typed `max_tokens` the encoder nests as `options.num_predict`, instead of losing the whole object to it (openai-chat-mapping.md §2.1.1, bl-f19d). The output cap and the context size stay distinct fields, and a size nobody stated is never fabricated — brazen emits no `num_ctx` unless the row or the request names one. A request value wins by being present; among config layers, a flag beats the row default; the row default beats the encoder's bare default (`stream:false`). A provider whose row pins nothing leaves the field `None`/absent, and `encode` omits it — brazen **never burdens the caller with a value the model needs** (the row supplies it) nor **invents one the model doesn't** (an unpinned, unrequested param stays absent). This generalizes architecture.md §13.1.
...
- **Rows emit in `providers` order — the dump round-trips *priority*, not just content.** Row order is routing priority (§2.2, architecture.md §4.3.1), so a dump that reordered rows would silently re-route the config it claims to reproduce. Emitting the list as it stands makes `parse(dump(cfg))` preserve the order by construction, and it needs no order key on the wire because the `[[provider]]` array's line order *is* the encoding (§2.2). This **replaces** the narrower rule that sorted rows by name and floated the `default_provider` row to the front — enough to round-trip a head, not a list.
...
- **`effort`, `priority` and `image` are the row's TUNING capability, computed from the row and never stored** (bl-50a5; `image` bl-842b, answering "can I point `--image` at THIS row?" over the canonical key `image`). Two booleans answering "can I point `--reasoning` / `--tier` at THIS row?", each the conjunction of two data reads: the dialect's own `Protocol::tuning()` declaration — a DATA method beside `models_shape()`/`framing()`, living next to the `encode` that implements the projection — AND the absence of the canonical key (`reasoning` / `service_tier`) from the row's `unsupported_body_keys`. That is the same pair `strip_unsupported` enforces on the data plane (§4.1.1), so the listing cannot disagree with what a run does, and it is severable in the usual direction: deleting the row's decline restores the capability with no code edit. `Protocol::tuning()` has **no default impl** — a new dialect must answer for itself, where a default would silently claim (or deny) a capability its `encode` never implemented — and the claim is proved against each dialect's own `encode` key-agnostically (`src/tests/protocol_tuning.rs`: setting the knob changes the encoded request **iff** the dialect declares it projected). Every shipped dialect projects effort, so in practice `effort` is the per-row decline and `priority` is the dialect fact (providers.md §6.2: the OpenAI family and Anthropic have a lane, Google/Ollama/claude_code do not). **Why the listing owns it:** a consumer above brazen otherwise keeps its own copy of the protocol table to gate a control it offers, which is exactly the duplicate-fact drift this repo refuses; served here, that copy is deleted.
- **`tools` and `multi_turn` are the row's SHAPE capability — what its dialect can CARRY at all — and they have NO per-row operand** (bl-5053). One level up from the tuning pair: `effort`/`priority` ask "does this knob reach the wire?", these ask "does the request's own shape reach the wire at all?". A dialect that cannot carry a shape REJECTS it at encode (`ParseInput`/64, architecture.md §3.1 — never a silent drop, never a fabricated transcript), and before this column that refusal was visible only at CALL time, to the one caller that had already built the request: a host validated a tool-bearing worker role against the `claude-code` row because the row exists in the listing, and every session call then died at encode (bl-68ad). Same mechanism as `tuning` — a `Protocol::shapes()` DATA method beside `models_shape()`/`tuning()`, no default impl (a new dialect must answer for itself), proved against each dialect's own `encode` key-agnostically in `src/tests/protocol_shapes.rs`: the shape encodes **iff** the dialect declares it, and a refusal that is not `ParseInput` fails the proof too. **Why there is no `unsupported_body_keys` half:** that key is a *strip* (§4.1.1), and stripping a tool declaration would be exactly the silent semantic change architecture.md §3.1 forbids — so the only honest per-row decline would be a second reject arm with no wire vocabulary behind it. The dialect is the one home, the asymmetry with `tuning` is the design, and a row naming `tools` in `unsupported_body_keys` declines nothing (pinned by `list_providers_columns`). **Why bools and not a widened value:** the "carry the fact, never a lossy proxy" rule bites where a bool projects away a value that exists — as it would for `device`'s flow style. Here no value exists: `encode` accepts the shape or returns `ParseInput`, and *which* reject arm fired is a property of one request, not of the dialect. Every shipped HTTP dialect carries both; `claude_code` carries neither (claude-code.md §4.1, §4.2), which is the whole content of the column today.
- **`device` is the row's HEADLESS-SIGN-IN capability, and it carries the flow STYLE rather than a bool** (bl-6680). `bz --login` has two flows and only one of them completes from a machine with no browser (auth.md §7.3); which a row serves is decided by whether it declares a `device` block, and *which wire* that block speaks is `device.style` (`rfc8628` — the default — or `codex`, OpenAI's pre-standard variant, auth.md §10.8). The column is that value, `-` for a row with no block, and it is a data read of the row like every other column: delete the block and the capability goes with it, no code edit. **Why the style and not `device = true`:** a bool is the lossy projection of a fact that has a value (architecture.md's carry-the-fact rule), and the consumer this column exists for — a host above brazen deciding whether to hand `--login` a `--browser` it cannot serve — reads the value, not the presence. Nothing in brazen branches on the spelling outside `flows`' own dispatch, so widening the vocabulary with a third variant costs the column nothing.
...
**Output is the resolved `OutMode`, exactly as `--list-models` folds it** (model-discovery §2): `--json`, `BRAZEN_OUTPUT=ndjson`, and a config-file `output = "ndjson"` all select the object form. Text is the seven columns space-padded to the widest value, one row per line, no header — greppable, `awk`-able, and the same "one line per listed thing" shape `--list-models` prints. Both go to stdout; errors to stderr.

**Each capability GROUP renders as ONE column** naming the members the row has, `-` for none — `tuning` (`effort,priority`) and `shapes` (`tools,multi_turn`) — because the text table and the object are two renderings of ONE `Row` and must not name different facts; a text table that silently dropped a served fact would make `--json` the only honest read. `grep priority` and `grep tools` are the questions an operator asks; four bare `true`/`false` columns under no header are not. One rendering serves both groups (`render::named`), so a third group is a call and not a copy. Both sit **before** `credential` because `credential` is the one column whose value can contain a space (`not required`), so it stays last and whitespace-splitting a line keeps working.

```
$ bz --list-providers
openai-chatgpt        openai_responses      oauth2   effort,priority  tools,multi_turn  codex    stored
local                 ollama_chat           none     effort           tools,multi_turn  -        not required
...
The last line is the listing earning its keep: a row that reasons, needs no credential, and can carry neither a tool declaration nor a transcript.

There is **no per-row `Model` metadata and no `base_url` column.** The listing names rows; `--list-models --provider <id>` is the next read, and `--dump-config`/`data/defaults.toml` carry the row's full body. The set answers "which row, speaking what, needing what, tunable how, and can I reach it" — every member is a fact ABOUT the row that a run would act on, computed, never a slice of the row's body. That boundary, not the count, is what keeps the listing from reimplementing the dump: this section once said "four facts, a fifth would start reimplementing the dump", and the fifth (bl-50a5), sixth (bl-6680) and seventh (bl-5053) are admitted precisely because they are computed capability rather than authored configuration. Note what the sixth is NOT: the `device.url` is authored configuration and stays out; the *style* the row would speak is the capability, and that is what is listed. The seventh is the purest case — its two booleans are read off the dialect and touch the row's body not at all.
...
```rust
impl PartialConfig {
    /// `cache` is the discovery seam step 3b reads; `None` is a path that carries no
    /// such seam (`--login`, which names its provider outright, so routing never asks).
    fn into_resolved(self, req_model: Option<&str>, cache: Option<&dyn ModelCache>)
        -> Result<ResolvedConfig, ConfigError> { … }
}
```

The model used for routing is **request.model if present, else `getConfigValue("model")`** (architecture.md §4.3: "the request's model, when set, wins for routing"), and an **empty** result is ABSENCE, not a seed: `--model ""` / `model = ""` routes like the zero-config `bz "q"` (step 2), the same empty-input dissolve `cfg.model = ""` already spells downstream, so neither tier below is ever asked to place an empty string. Routing then resolves a single provider row by this query:

1. If a provider is named (`provider` field, from flag/env/file) → find the row of that name in `providers`.
2. Else, **with no routing model at all** (the zero-config `bz "q"`) → default to `providers.first()`: config-file order, "whatever you find first reading from the top," **not** the alphabetically-first name. A **user file's rows outrank the built-in defaults'** (a config declaring `chatgpt` first defaults to `chatgpt`, never the built-in `anthropic` beside it in the merged table) because the merge puts the higher layer's rows first (§3.2). The empty model seed then walks `select_model`'s ladder in `serve` (model-discovery.md §4.1: last-used, else the row's first cached model) — "no specification" resolves to (first row, that row's ladder). **Routing is unaffected**: last-used is model-selection only, never a provider tiebreak. A config with **no provider rows at all** (an empty `providers`) is the lone `NoProvider` residue here. (`--login` opts out of this default — §7.1 below / auth.md §7.1 — a credential write must name its target via the *selector*.)
3. Else (a routing model is present, no provider named) → the list is read **once per ownership tier**, and the first owner found wins:
   - **3a — CLAIMS.** `providers.iter().find(|(_, row)| row_claims(row, model))`: the **first** row whose `model_aliases` **contains** the model (substitution shorthand) **or** one of whose `model_prefixes` is a prefix of it (family ownership). Either match claims; `find` **stops at the first**, so further owners are never consulted and are **not** an error (architecture.md §4.3 — the priority list is the tiebreak). Prefix ownership is what lets `bz --model claude-haiku-4-5-20251001 "q"` route with no `--provider` — a versioned wire id no alias could enumerate is routed by the family its row claims (e.g. anthropic ships `model_prefixes = ["claude-"]`). `openai-responses` and `ollama` ship none (the former shares OpenAI's ids over a second protocol; the latter has no stable family), so they stay opt-in via explicit `--provider`.
   - **3b — OBSERVED MODELS (the fall-through).** Only if 3a found nothing: `providers.iter().find(|(name, _)| cache_places(cache, name, model))`, where a row's cache places the model iff `select_model(cache.get(name), model, name)` returns `Provenance::Cached` — the **same** exact-else-case-insensitive-substring rule `generate` will apply to the same seed against the same list (model-discovery.md §4). So `bz --model 5.5 "q"` with `anthropic` above `openai` routes to **openai**: anthropic's cached list cannot place `5.5`, openai's `gpt-5.5` can. A **local file read per row**, never a probe — a row with no cache file (or a run with `cache: None`) simply cannot win this tier, which is the documented cost of keeping resolution network-free (architecture.md §4.3.2).

   **The tiers are walked whole, and that is the precedence ruling:** every row's *claim* is checked before any row's *cache*, so a stale or learn-on-success-appended cache entry on an early row can never steal a family a later row **claims** (model-discovery.md §5.4 — the append is exactly how such an entry appears). A cache hit can only ever win a model **no row claims at all**. Within each tier, greedy first-match over declaration order is unchanged.

**Step 3 (both tiers) is the *only* order-sensitive resolution in brazen.** Step 1 (an explicit `--provider`) is a by-name lookup that ignores order entirely and remains the unconditional override for anyone who does not want to reason about priority at all.

**An empty-string `model_prefixes` element is `BadValue` (78) — the one validation greedy-first makes load-bearing.** `"anything".starts_with("")` is `true`, so a row carrying `model_prefixes = [""]` owns **every model**. Under the retired ambiguity rule such a row collided with any other owner and failed loudly; under greedy-first an early row with an empty prefix silently swallows all routing, and the operator sees no error — only the wrong provider, forever. An empty prefix is never an authored priority; it is a typo or a half-deleted entry, so it is refused at resolution rather than obeyed. **The distinction is exact and both halves matter:** `model_prefixes = []` — the empty *list*, claiming nothing — stays perfectly legal and is how a row opts out of family routing (`openai-responses` ships it); `[""]` — an empty *element* — is the error. This is the general rule that a value which cannot be meant should not be silently honored, not a special case for routing.

Every way this can fail, each → `ConfigError` → exit 78:

| `ConfigError` variant | Trigger | Why surfaced, not papered over |
|---|---|---|
| `NoProvider` | No provider named **and** EITHER a routing model owned by **zero** rows *on either tier* (no `model_aliases` entry, no `model_prefixes` match, and no row whose cached model list can place it), OR **no model at all and an empty provider table**. A no-model request with a **non-empty** table is NOT this — it defaults to the first row (step 2). `--login` re-raises it when no provider is named. | Cannot route. A *given* model no row owns needs explicit `--provider` (architecture.md §4.3 — ownership and identity passthrough cover *routing*/*substitution* respectively, not each other); with no model there is nothing to own, so the first row is the default, not an error. |
...
| *(retired — **no** variant: greedy-first resolves it)* | Two or more rows owning one routing model **is not an error.** The first in `providers` wins (step 3). | architecture.md §4.3: a total order always has a first element, so there is no tie to surface. The former `AmbiguousModel` refused instead, which forced an operator diverting **one** id to disarm a whole vendor's family routing — collateral worse than the ambiguity. The operator authored the order; `--dump-config` prints it (§6), and `--provider` overrides it. |
...
Resolution does **nothing further** about the model: there is no owned-vs-probe query and no `probe` field (this **dissolves** the former `ResolvedConfig.probe` — model-discovery.md §5). `model_prefixes` is consumed for **routing only** (which row owns a full id, step 3 above); whether the resolved `model` is a full wire id or a partial is **not** decided here. `cfg.model` carries the alias-substituted string **verbatim** — a full id, a partial seed, or `""` (absent) — and `serve` resolves it against the provider's model cache by the total `select_model` (an exact/partial match, the default, else the string verbatim — model-discovery.md §4–§5). Resolution stays pure **over its injected inputs** (§8): it performs no transport and opens no file itself — the `ModelCache` seam is passed in, and step 3b reads it exactly as `serve` does. Routing *does* now consult it: a partial that **no** row can place is still `NoProvider` (the table above), but one that a row's observed list places routes there (step 3b). (The former `row_has_prefixes`/bl-3989 probe guard is gone with the probe — no generation path makes a `/models` GET, so the prefix-less-row regression it patched cannot recur; model-discovery.md §5.2.)
...
| Every `ConfigError` (§7) | One literal case each: `NoProvider`, `UnknownProvider`, `IncompleteProvider`, `BadValue`, `MalformedFile` (incl. `deny_unknown_fields` and duplicate `name`). A valid-but-unregistered `protocol`/`auth` id is **not** a `ConfigError` — it fails closed at dispatch (§7), tested at the registry seam. |
| **Greedy-first routing** (§7 step 3) | Two rows owning one model resolves to the **earlier**, no error: by alias-over-prefix, by prefix-over-prefix, and by a **user row over a defaults row**. Plus the negative that proves the *order* decided it and not an accident of the structure: reorder the two rows and the winner flips — the executable form of "the priority list is the tiebreak" (architecture.md §4.3). |
| **Redeclaring hoists a row** (§3.2) | A user row naming a defaults row with only a `body_defaults` tweak resolves with the defaults' `model_prefixes` **at the user's position** — the foot-gun stated in §3.2, pinned so it cannot regress silently. |
| **The merge's tail law** (§3.2) | A user file + defaults merge keeps the defaults' **unredeclared** rows in defaults order, behind the user's rows — the third position law, which the greedy-winner and redeclare-hoist cases do not reach. |
| **Empty prefix element** (§7) | `model_prefixes = [""]` → `BadValue` (78), while `model_prefixes = []` resolves fine — the pair, since only the contrast pins the rule. |
| **Order round-trips `--dump-config`** (§2.2, §6) | Dump a config whose priority differs from name order, re-parse it, and get the same row order — the property the old default-row-floats-first hack could only give for the head. |
| `--dump-config` | Golden TOML: `merged_without_defaults`, secrets as `"<redacted>"`, rows in `providers` order with scalar maps in `BTreeMap` order; and the **round-trip** `parse(dump(cfg)) == merged_without_defaults` (§6, §2.2). |
...
- **`ConfigError` variants** (§7) refine architecture.md §8's single "78 = no provider resolved / unknown / bad config" row into the specific surfaced errors; the exit code and class are unchanged (all → 78).
