...
| `Document{source: Url{url}}` | **UNREPRESENTABLE** → `Error{ParseInput}`/64 (§6 CR-6). Unlike `image_url`, Chat Completions **file inputs accept no external URL** — only base64 `file_data` or an uploaded `file_id`. The asymmetry with images is a real wire fact, not a brazen choice; the reject message names the remedy (the Responses API, or send as a base64 document). |
