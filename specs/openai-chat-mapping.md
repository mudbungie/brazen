...
| `cache_key: Option<String>` | `"prompt_cache_key"` | The portable prompt-cache ROUTING knob (architecture.md §3.1), spelled out under this table. `Some(k)`→the string verbatim; `None`→omit. A lifted known knob; the Responses dialect spells it identically (providers §7.1). |
...
`parallel_tool_calls`, `output`, `service_tier` and `cache_key` are now typed canonical fields above (omitted → OpenAI's defaults). `n`, `seed`, `logprobs`, `presence_penalty`, `frequency_penalty` have **no canonical home** and reach the wire only via `extra`.
...
**`prompt_cache_key` (the SIXTH lifted knob, providers.md §7.1).** `req.cache_key` projects to `"prompt_cache_key": <the string, verbatim>`, written BEFORE the `extra` fold so the typed knob wins over a `body_defaults` `prompt_cache_key`. `None` omits it, and the body is byte-for-byte the pre-knob shape. **Why it must be sent:** OpenAI's automatic prefix cache is served by a POOL of replicas and routes by this key; brazen is stateless and resends the whole grown transcript every turn, so without it each turn lands on whichever replica takes it and reads back **zero** cached tokens even on a byte-identical, append-only prefix (bl-8b47, measured). The value is OPAQUE — brazen neither generates nor validates it; one id per conversation BRANCH, stable for the branch's life, chosen by the caller that owns branch identity. **Read right-to-left (the ingress inverse):** a STRING lifts onto the typed field so a re-routed request does not carry an OpenAI-only key to a dialect that 400s on it; anything else has no canonical home and rides `extra` verbatim (the same rule as `service_tier`'s unmodelled lanes — the wire slot exists, and judging the VALUE is provider policy, ingress.md §3).

#### 2.1.1 `extra` precedence (single source of truth)

`encode` serializes the typed canonical fields **first**, then folds in `extra` keys that are **not already set by a typed field** — the **typed field wins** (it is the single source of truth; `extra` is the long-tail valve, architecture.md §3.1). `extra` MUST NOT override a field derived from a typed canonical field. **Two objects merge one level** (bl-f19d): when a key is an object on BOTH sides, the passthrough's second-level keys fold in beneath the typed ones rather than the whole object being dropped — the typed value still wins per second-level key, and a key the encoder never wrote survives beside it. That is what makes the valve reach a NESTING dialect at all (Ollama's `options`, Google's `generationConfig`): before it, a row's `body_defaults = { options = { num_ctx = N } }` was dropped whole the moment any typed gen scalar was set, which is every agent request. It stops at one level because a body map is a NAMESPACE of fields and each dialect nests its generation params exactly one namespace deep, while below that sits a VALUE the encoder owns whole (a JSON Schema under `output_config.format`, a tool's `parameters`) — merging two schemas key-by-key yields a chimera that is neither. One home in code: `protocol::json::fold_extra` (protocol-dedup.md D1). This is the **same precedence rule as the Anthropic messages mapping (anthropic-messages.md) §2.1.1** — the two protocol adapters give `extra` identical precedence, and it avoids the `#[serde(flatten)]` duplicate-key hazard. The `max_tokens`-vs-`max_completion_tokens` key-selection is **not** an `extra`-override case: `encode` selects the key from the typed `reasoning` signal (§2.7), and the non-reasoning row-config path resolves at the row/resolution layer.
...
| `Image{source: Base64{media_type,data}}` | `{type:"image_url", image_url:{url:"data:{media_type};base64,{data}"}}` — base64 embedded as a **data-URI string** inside `url`. Chat Completions has **no** structured `{media_type,data}` image source (§6 CR-C1). |
| `Image{source: Url{url}}` | `{type:"image_url", image_url:{url}}` |
| `Document{source: Base64{media_type,data}}` | `{type:"file", file:{filename:<synth>, file_data:"data:{media_type};base64,{data}"}}` — base64 as a **data-URI string** inside `file_data` (like `image_url`). Chat **requires** `filename` on `file_data`, which the canonical `DocumentSource` lacks, so it is **synthesized** as `document.{subtype}` from the media type (`application/pdf` → `document.pdf`). Documents are **input-only** in **user** turns (a document in a text-only slot rejects, §2.3). |
| `Document{source: Url{url}}` | **UNREPRESENTABLE** → `Error{ParseInput}`/64 (§6 CR-C6). Unlike `image_url`, Chat Completions **file inputs accept no external URL** — only base64 `file_data` or an uploaded `file_id`. The asymmetry with images is a real wire fact, not a brazen choice; the reject message names the remedy (the Responses API, or send as a base64 document). |
...
| `Thinking{text, signature}` | **dropped** — no request representation in Chat Completions (§2.9, §6 CR-C2). |
...
- `content` ← `content` flattened to OpenAI tool-message content (string or array of `text` parts only; tool messages accept no `image_url`). `ToolResult.content` stays a permissive `Vec<Content>` canonically (architecture.md §3.1); a non-`Text` `Content` nested inside it that cannot be represented as tool-message text is a runtime degradation surfaced as `Error{kind: ParseInput}` (→ 64), the same text-only-slot rejection as §2.3 (architecture.md §3.1; §6 CR-C5).
- `is_error` ← **no native field** (§6 CR-C3). The flag cannot round-trip structurally; the adapter surfaces it **textually** by prefixing the content (e.g. `"[error] "`) so the model still sees the error signal.
...
**Reasoning** (`delta.reasoning_content` — the openai-compatible class's extension, §3.3a). On the first chunk with a **non-empty** `delta.reasoning_content`: **synthesize** `ContentStart{index: i, kind: ContentKind::Thinking{ id: None }}` where `i = state.next_index++`, insert `i` into `state.open`; then for that and every subsequent non-empty fragment emit `ContentDelta{index: i, delta: Delta::ThinkingDelta(fragment)}`. Exactly the lazy-block discipline of **Text** below (identity before content; an empty `""` opens nothing), one field over. **Absent** — stock OpenAI and every non-reasoning backend — it decodes nothing: the empty-set path, no branch. The ruling and its rationale are §3.3a.
...
### 3.3a The reasoning channel — `reasoning_content` → `Thinking` (the ruling, bl-b68b)

**The OpenAI specification has no reasoning field on this endpoint.** Stock reasoning models (o-series/gpt-5) *spend* reasoning tokens on Chat Completions but return **no reasoning text**; the readable channel exists only on the Responses API (`response.reasoning_summary_text.delta`, openai-responses-mapping §3.4). But this one row serves the whole **openai-compatible class** (providers.md §9 / `data/defaults.toml` invite arbitrary compatible backends onto it), and that class converged on a de-facto field: **`reasoning_content`** — on `choices[0].delta` while streaming (DeepSeek `deepseek-reasoner` and the servers that copy it), and on `choices[0].message` in a `stream:false` body.

**Ruling: decode `reasoning_content` to the canonical thinking channel.** It is the same lazy synthesized block the text channel already gets, differing only in `kind`/`Delta` (§3.3), so it costs no new mechanism: the canonical `ContentKind::Thinking{id:None}` / `Delta::ThinkingDelta` and the `--thinking` sink already exist (architecture.md §3.2), and `protocol::synth::open_thinking` is the shared lazy-open the Google `thought` and Ollama `thinking` channels use (providers.md §4.4, §5.5). Decoding an extension the base spec does not name is **not** per-vendor branching here, because it is **inert when absent** — a backend that never sends the field takes the empty-set path and emits exactly what it emits today. The alternative (drop it) is the strictly worse asymmetry the sibling structureless dialects already refused: reasoning text arrives on the wire, brazen holds a canonical home for it, and dropping it loses output the user paid for.

- **`id` is `None`.** Chat Completions carries no reasoning-item id (only the Responses API does), and adapters never fabricate one (architecture.md §3.1).
- **Index order is first-seen, not fixed.** Reasoning precedes the answer on every observed backend, so the thinking block is normally index 0 and the text block index 1; nothing depends on that — the general lazy-open rule assigns whichever arrives first, and both drain ascending at the finish chunk (§3.3).
- **No replay.** A decoded `Content::Thinking` from this dialect has no `signature` and is **dropped on re-send** (§2.9, §6 CR-C2) — unchanged by this ruling. Decode surfaces the text; it does not make Chat Completions a thinking-replay transport.
- **Only `reasoning_content` is decoded.** Other spellings exist in the wild (OpenRouter's `delta.reasoning`, a structured `reasoning_details[]`). They are **not** decoded: one field with a settled meaning (a plain text fragment) is the convention; the rest are per-vendor guesswork. Each is one line and one fixture away if a live backend ever forces it — a spec change here, never a config knob.

**The stock-OpenAI silence is CORRECT and gets no notice.** A `--reasoning`-bearing turn against a backend that returns no reasoning text renders nothing under `--thinking`, and brazen says nothing about it. `req.reasoning` on this dialect is **not** a lossy adaptation: it is projected faithfully (`reasoning_effort`, plus the §2.7 field adjustments), the upstream honors it, and the tokens are spent and billed — **nothing brazen holds is dropped**. What is missing is a channel the *provider* does not expose, which is the provider's court, not brazen's ("brazen carries every dialect's spec; it carries no provider's water", ingress.md §3). The named-adaptation vocabulary (`lossy_overrides`, the `brazen.adaptations` response field, the `: brazen adaptation=…` SSE comment — ingress.md §4) is the **ingress** mechanism for a request *brazen itself* downgraded at the masquerade edge; an ordinary egress run has no adaptation carrier, so borrowing the name would mean building a new one to announce a non-loss. So: no flag, no notice, no adaptation name. Empty thinking output is the honest report that there was no thinking text.
...
| `openai_chat_reasoning` | A compat-class reasoning stream (`delta.reasoning_content` fragments, then `delta.content`, §3.3a). Asserts `ContentStart{0,Thinking{id:None}} → ThinkingDelta* → ContentStart{1,Text {}} → TextDelta → ContentStop{0} → ContentStop{1} → Finish{Stop}`: the thinking block opens lazily ahead of the text block, the role-only `reasoning_content:""` opens nothing, and a `null` fragment decodes nothing. |
| `openai_chat_nonstream_reasoning` | The `stream:false` mirror: `choices[0].message.reasoning_content` projects onto the synthetic delta, so the folded body yields the SAME thinking+text block pair (§3.3a) plus the same-frame `Usage` after `Finish`. |
...
- **`Thinking` IS produced — but only from `reasoning_content`, and only one-way.** The compat class's `reasoning_content` decodes to `ContentKind::Thinking{id:None}` (§3.3a); stock OpenAI never sends it and the decode is inert there, with **no notice for the silence** (nothing brazen holds is dropped — §3.3a's ruling). The decoded block still **drops on re-send** (§2.9, CR-C2): decode-side surfacing and encode-side replay are independent.
...
**Architecture change requests (raised, scoped, NOT silently worked around).** This spec's own change requests are the **`CR-C*`** namespace (OpenAI Chat); the shared architecture-owned CRs (`CR-4` serde/server-tools, `CR-9` terminated/premature-EOF, `CR-10` mid-stream exit-by-`kind`) keep their bare numbers. Sibling per-spec namespaces: `CR-A*` (anthropic-messages.md), `CR-R*`/`CR-G*`/`CR-O*` (providers.md).

- **CR-C1 — `Content::Image{Base64}` is structurally compressed but faithful.** Chat Completions has no structured `{media_type,data}` image source; base64 is embedded as a data-URI `data:{media_type};base64,{data}` inside `image_url.url`. This **round-trips** (decode re-parses the data URI back to `Base64{media_type,data}`), so **no architecture change is required** — recorded as a watch item only.

- **CR-C2 — `Content::Thinking` has no request representation on Chat Completions.** `signature` cannot round-trip; the adapter **drops** `Thinking` (and the opaque `RedactedThinking`) on re-send (§2.9). Consistent with architecture.md §3.1 (signature `None` / never fabricated; redacted-thinking never produced by a non-Anthropic adapter). **No architecture change requested for v0.1** — thinking replay rides the Responses API / Anthropic. A change request is raised **only if** future requirements demand thinking replay *through Chat Completions*.

- **CR-C3 — `Content::ToolResult.is_error` has no native Chat Completions field.** OpenAI tool messages carry no error flag. The adapter surfaces it **textually** (content prefix, §2.4) so the signal survives, but the structured boolean does **not** round-trip. **Change request to the architecture spec:** *if* a structured tool-result error channel must ever survive a Chat Completions round-trip, the architecture spec should bless the degradation rule ("textual surfacing, no canonical change") explicitly so this adapter is not silently lossy. Until then, a documented, intentional degradation.

- **CR-C6 — `Content::Document{Url}` is UNREPRESENTABLE on Chat Completions (asymmetric with images).** Chat Completions `file` inputs accept **only** base64 `file_data` or an uploaded `file_id` — **not** an external URL, unlike `image_url`, which passes a web URL through. So `Document{Base64}` maps (data-URI `file_data`) but `Document{Url}` **rejects at `encode`** with `ErrorKind::ParseInput` (exit 64), the §3.1 unrepresentable-slot rule; the message names the remedy (the Responses API, whose `input_file` DOES take a `file_url` — providers.md §3.3, or send as a base64 document). The image analogue does not apply — this is a document-specific wire narrowing, one dialect's own limit, no canonical change. A second wrinkle: chat **requires** a `filename` on `file_data` that the canonical `DocumentSource` does not carry, so the adapter **synthesizes** `document.{subtype}` from the media type (a mechanical, deterministic default — no new field). **No architecture change requested.**

- **Server tools (architecture.md CR-4, resolved there — this adapter's documented degradation).** The canonical `Tool::Provider{kind,name,config}` and `Content::ServerToolUse`/`ServerToolResult` (architecture.md §3.1) are Anthropic-carried opaque passthrough. This adapter does **NOT** project them: a `Tool::Provider` in `tools[]` **rejects at `encode`** with `ErrorKind::ParseInput` (exit 64, "provider-typed tools are not projected for this dialect") — fail fast, never a silent drop; a `Content::ServerTool*` block in a transcript hits the existing non-representable-content rejection the same way (`ParseInput`/64). Server-tool RESULT blocks are likewise never decoded/surfaced here (Chat Completions has no such wire block — the empty-set rule). `Tool::Custom` is unaffected (§2.5). The OpenAI **Responses** dialect DOES carry them, verbatim, as its native typed tools (providers.md §9, bl-83b4); Chat Completions has no such wire slot. No architecture change requested.

**Resolved in architecture.md (the serde flag is architecture.md's shared CR-4; this spec's CR-C5 non-text-slot flag resolved there too — recorded here for provenance, no longer open):**
