...
| `tools: Vec<Tool>` | `"tools"` | **omit when empty.** A custom tool → `{type:"function", name, description?, parameters}` — **flat** (no nested `function` envelope, unlike Chat Completions). A provider-typed tool → carried **verbatim** as a native typed tool, `{type, name?, ...config}` (§9 server-tools bullet, bl-83b4). |
...
| `response.output_item.done` with `item.type=="image_generation_call"` (the `image_generation` tool's output: `{id:"ig_…", status, result:<base64>, output_format?:"png"\|"jpeg"\|"webp", …}`) | **synthesize all three from this one frame**: `ContentStart{index, Image{ media_type: "image/"+output_format }}` (`png` when absent — the tool's documented default), then ONE `ContentDelta{index, ImageDelta(result)}`, then `ContentStop{index}` (bl-0987, architecture.md §3.2). Identity-before-content holds inside the emitted vec. The matching `output_item.added` frame carries `result: null` and opens nothing — the image is not in-band until done, so a block opened early would have no faithful identity | assign pair `(output_index, 0)` a canonical index; opened and closed in the same call, so it never enters `open` |
| `response.image_generation_call.partial_image` / `.in_progress` / `.generating` / `.completed` | **no-op** — a `partial_image` is a complete progressive RENDER (`partial_image_b64`), not an appendable fragment, so it has no place in an append-only delta stream (architecture.md §3.2 point 2); the final `result` on the done item is the image. `--raw` reaches the partials | fall through, yield `[]` |
...
| `parts[].inlineData{mimeType, data}` (a model-RETURNED image — the image models answer this way when `generationConfig.responseModalities` includes `"IMAGE"`; arrives **whole** in one chunk like `functionCall`) | **synthesize** `ContentStart{c, Image{ media_type: mimeType }}` then one `ContentDelta{c, ImageDelta(data)}` (bl-0987, architecture.md §3.2); the block stays open and **closes at the terminal drain**, the `functionCall` row's discipline. Before this row the part fell through the loop and the image was silently dropped | assign canonical index (monotonic `open.len()`) |
...
### 4.5 Tool-call id is synthesized (Google's, when present, is ignored); the result is keyed back by NAME

Google matches a tool RESULT to its CALL by function `name`, never by id (request side below). Historically `functionCall` carried **no id** at all; a live gemini-3-flash-preview capture (bl-34b3, the `google_genai_tools.sse` golden) shows current models **do** emit a `functionCall.id` (e.g. `"02sja778"`) — but the adapter **deliberately ignores it** and keeps synthesizing its own. The canonical model requires `ContentStart{ToolUse{id, name}}` with a non-empty `id` so identity-before-content holds and a folding consumer can key the call (architecture.md §3.2, §3.6); the adapter **synthesizes a deterministic id** — `"call_{candidateIndex}_{block_index}"` from `DecodeState` — which is stable whether or not Google supplies one and is redundant with Google's, since results are name-keyed. Adopting Google's id would trade a deterministic, self-consistent id for a provider-variable one with no wire benefit, so the synth id stays authoritative.
...
`usageMetadata`: `promptTokenCount`→`input_tokens`, `candidatesTokenCount`→`output_tokens`, `cachedContentTokenCount`→`cache_read_tokens` (`Some` iff present), no cache-write → `cache_write_tokens: None`. `promptTokenCount` is documented as "still the total effective prompt size meaning this includes the number of tokens in the cached content", so `input_total_tokens` is it unchanged (architecture.md §3.2). `totalTokenCount`/`thoughtsTokenCount` derivable/long-tail → dropped. `Option` throughout — absent field is `None`, never `0` (architecture.md §3.2).
...
### 4.10 Google Cloud Code (the Antigravity backend) — `mod google_cloudcode`: the §4 wire in a two-key envelope (bl-cbd4)

**Why a second Google dialect.** An individual Google subscription no longer reaches Gemini
through the Gemini API's own sign-in (auth.md §11.0); it is served by the Cloud Code backend
the Antigravity client speaks, which carries the SAME `GenerateContentRequest`/`Response` as
§4.2–§4.4 inside an envelope. Everything Google-shaped is reused; the module owns only the
envelope, the path and the unwrap. Facts from router-for-me/CLIProxyAPI, verified live
2026-09-25 (auth.md §11.4).

**The row** (an operator recipe, auth.md §11.2): `base_url = "https://daily-cloudcode-pa.googleapis.com"`,
`protocol = "google_cloudcode"`, `auth = "oauth2"`, bearer `Authorization`. The prod host
`cloudcode-pa.googleapis.com` answers generation with 429; `daily-` serves it.

**REQUEST.** `POST {base_url}/v1internal:streamGenerateContent?alt=sse` (stream) or
`/v1internal:generateContent` (non-stream) — **no model segment in the path**. Body:

```json
{"model": "<model>", "request": <exactly §4.2's generateContent body>}
```

The inner body IS `google_genai`'s `body_map` (made `pub(crate)`, not copied): system hoist,
`contents`, tools, `generationConfig`, and the one `extra` fold — so `extra`/`body_defaults`
land INSIDE `request` exactly as they do on §4. The envelope's keys are protocol-owned and
there are two. The client also sends `project`, `requestType`, `requestId`, `userAgent` and a
`sessionId`; **all verified unnecessary** — a bare `{model, request}` streamed text and a 1.5 MB
image (auth.md §11.4) — so none is emitted (the empty-set rule; CR-CC records the door).

**The license gate is a header, not a body fact.** The backend keys entitlement on a
`User-Agent` beginning `antigravity` (`ureq/3` → `403 SUBSCRIPTION_REQUIRED`). That is an
auth-mode-dependent static header, so it rides the OAuth row's `beta_headers` (auth.md §4) and
this module knows nothing of it.

**RESPONSE.** Each SSE `data:` chunk (and the whole non-stream body) is
...
item). Leading `{"thought":true,"text":""}` parts are skipped by the existing `nonempty` guard.

**Declined, as data.** `models_shape() = None`: the listing is `POST :fetchAvailableModels`
returning a name-keyed MAP (`GET` is 404), neither the method nor the array shape the one
generic `decode_models` reads — deferred (CR-CC). `count_tokens` = the default decline
...
| `extra` | merged top-level / into `options` | the valve: `keep_alive`, `format` (JSON-mode/schema), `options.*` knobs (`num_ctx`, `seed`, `repeat_penalty`, …). Typed fields win (architecture.md §3.1) — per key, since the fold merges an `options` object one level (openai-chat-mapping.md §2.1.1). **`num_ctx` is the context window and has no canonical field**: brazen never states it, so a request runs at the server's own default unless a row pins `body_defaults = { options = { num_ctx = N } }` — which now composes with the typed `max_tokens` the encoder writes as `options.num_predict` instead of being dropped by it (bl-f19d). The two are distinct fields: `num_predict` caps the OUTPUT, `num_ctx` sizes the INPUT window. A pinned `num_ctx` is also the window the turn's `usage` events report (model-discovery.md §5.5): it is the size the server will actually allocate, so it outranks any capacity the model list states. |
...
|---|---|---|
| `openai_responses` | `"reasoning": {"effort": e.as_str(), "summary": "auto"}` (both channels — the readable summary AND, via `include`, the encrypted replay blob; §3.2, bl-f90e) | §3.2 |
...
**The SEVENTH lifted knob, `image`, is §6.3.** **The SIXTH lifted knob lives with its subject.** `cache_key` (§7.1) is lifted by exactly this reasoning, but it is a prompt-caching fact, so it is documented once in §7 beside the placement policy rather than a second time here.
...
### 6.3 Image output (`--image`) — one canonical knob, two wire spellings and four narrowings

`CanonicalRequest.image: Option<bool>` (architecture.md §3.1, bl-842b) is a portable IMAGE-OUTPUT intent — "the reply may include an image" — that each `encode` projects to its dialect's own spelling, or rejects. It folds like `--tier`: request > `--image` > `BRAZEN_IMAGE` > config-file `image = true` (config §4). `None` and `Some(false)` are the same absent fact on the wire: nothing is written.

**A bool, not an enum.** The intent has no value beyond present/absent. The exact modality list and the tool's own configuration (`size`, `quality`, `partial_images`, …) stay reachable verbatim — a `generationConfig` object via `body_defaults`/`extra`, a configured `{"type":"image_generation", …}` via `tools` (`Tool::Provider`, §9) — and the typed knob composes with both rather than replacing them.

**Per-protocol projection** (each owned by that protocol's `encode`; the typed knob is written BEFORE the `extra` fold, so it WINS on a same-named key):

| Protocol | `Some(true)` | Spec home |
|---|---|---|
| `google_generative_ai` | `generationConfig.responseModalities: ["TEXT","IMAGE"]` | §4.2 |
| `google_cloudcode` | the same, INSIDE `request` — inherited through the shared `body_map`, no code of its own | §4.10 |
| `openai_responses` | append `{"type":"image_generation"}` to `tools` (the key appears even when `req.tools` is otherwise empty) — UNLESS `req.tools` already holds a provider tool of kind `image_generation`, whose own configuration then stands alone (never a duplicate) | §3.2 |
| `anthropic_messages` | **REJECT** — `ParseInput`/64: the wire returns no images | anthropic-messages.md §2 |
| `openai_chat` | **REJECT** — `ParseInput`/64: Chat Completions returns no images | openai-chat-mapping.md §2 |
| `ollama_chat` | **REJECT** — `ParseInput`/64 | §5.3 |
| `claude_code` | **REJECT** — `ParseInput`/64 | claude-code.md §3 |

**Reject, not omit.** `--tier` narrows SILENTLY on Google/Ollama because a missing lane changes only speed; a missing image changes the ANSWER, so a dialect that cannot return one says so (architecture.md §3.1, the loud-narrowing rule). A row whose backend declines the knob lists the canonical key `image` in `unsupported_body_keys`, and brazen strips it pre-encode (config §4.1.1) — the same severable opt-out every lifted knob has. `Tuning.image` declares the projection beside each `encode`, and `bz --list-providers` shows it in the `tuning` column as `image`.

**The model is the caller's.** `--image` never selects a model: Google needs an image model (`gemini-3.1-flash-image`, `gemini-2.5-flash-image`), and choosing one from the flag would mean reading meaning into a model NAME — the structural sniffing architecture.md §2 forbids. A row alias (`ag-image = "gemini-3.1-flash-image"`) is the ergonomic answer, and it is config.

**Verified live 2026-09-25** through the installed `bz` (§9 CR-Img): `bz --provider antigravity -m ag-image --image "…"` and `bz --provider openai-chatgpt -m gpt-5.5 --image "…"`.
...
- **CR-Img — model-RETURNED images decode on the two dialects that produce them; every other dialect is the empty set (bl-0987, architecture.md §3.2).** Before this ball the canonical `image` block was input-only and BOTH producing decoders dropped a returned image silently — Google's part loop had no `inlineData` arm (§4.4) and Responses' `item_added` yields `[]` on an unknown item type (§3.4) — the one direction the loud-never-silent media rule (architecture.md §3.1) did not cover. Now: **Google** `parts[].inlineData` → `ContentKind::Image{media_type: mimeType}` + one `ImageDelta` (§4.4), requested through `generationConfig.responseModalities:["TEXT","IMAGE"]` riding `extra`/row `body_defaults` (`fold_extra` merges object keys, so it composes with the typed `generationConfig`); **OpenAI Responses** `image_generation_call` done item → `Image{media_type: image/<output_format>}` + one `ImageDelta(result)` + stop (§3.4), requested by the provider-typed tool `{"type":"image_generation", …}` (`Tool::Provider`, passed verbatim) — carried since bl-83b4; before it the Responses encoder rejected every provider tool, so this path was specified but unreachable. **The request is a lifted knob, `--image`** (§6.3, bl-842b — reversing this bullet's original "no lifted knob" ruling, which forced callers to hand-build the body). **Empty set, stated:** Anthropic Messages, OpenAI Chat Completions, Ollama `/api/chat` and `claude_code` return no image block, so their decoders gain nothing; the ingress codecs (ingress §12) likewise skip the kind, since neither client wire has an image OUTPUT block — `--json` on egress is the only surface that carries it. **Replay** is a `user`-turn `Image{Base64}` (`-f bz-<hash>.png`), the stateless editing pattern both producing wires document; no assistant slot changes. **Verification — LIVE on both producing dialects (2026-09-25, through the installed `bz`):** **Google** via the `antigravity` recipe row (§4.10, the Cloud Code envelope — the operator's AI Studio key is free tier with image quota 0) with `gemini-3.1-flash-image` and `generationConfig.responseModalities:["TEXT","IMAGE"]` riding `extra`: `content_start{image:{media_type:"image/jpeg"}}` + one `image_delta`, and `--text` wrote a 1408×768 JPEG named by its hash; **OpenAI Responses** via the `openai-chatgpt` row with `gpt-5.5` and `tools:[{"type":"image_generation"}]` (reachable only since bl-83b4): `content_start{image:{media_type:"image/png"}}` + one `image_delta` + stop, then an empty text block, and `--text` wrote a 1536×1024 PNG. Both images were the requested shape. The fixture-driven decoder tests stand as the regression net.
- **CR-CC — Cloud Code (Antigravity) doors left closed, with the evidence (bl-cbd4, §4.10).** (1) **`project`/onboarding:** the native client learns a `cloudaicompanionProject` via `loadCodeAssist` (`{metadata:{ideType:"ANTIGRAVITY"}}` → `"aicode-consumers"` for the operator's free tier) and sends it in the envelope; generation was verified to succeed WITHOUT it, but only after that account had been touched by the probe's `loadCodeAssist`, so a never-onboarded account may 403 once. The one-off is a `POST https://cloudcode-pa.googleapis.com/v1internal:loadCodeAssist` with that body and the bearer + `User-Agent: antigravity` (then `POST daily…/v1internal:onboardUser {tier_id:<allowedTiers[isDefault].id>, metadata:{ide_type:"ANTIGRAVITY",ide_name:"antigravity",ide_version:"2.15.1"}}` polled until `done`, if no project comes back). A login-time RPC step and an envelope `project` key are the mechanisms NOT added until that case is real. (2) **Models listing:** `POST :fetchAvailableModels` → `{models:{"<name>":{displayName,quotaInfo,…},…}}` — a map keyed by name; the generic GET+array reader cannot take it, so the row declines `--list-models`; the 33 names seen include `gemini-2.5-flash`, `gemini-2.5-pro`, `gemini-3.1-flash-image`, `gemini-3.x-flash-*` tiers, `claude-sonnet-4-6`, `claude-opus-4-6-thinking`, `gpt-oss-120b-medium`. (3) **`requestType:"image_gen"`/`requestId`/`sessionId`:** verified optional for both text and image; not emitted. (4) **The `thoughtSignature` on an `inlineData` part** (~900 KB) has no canonical replay slot — the decoder ignores it; image-edit replay of a generated image would need it and is a later design pass. (5) **Non-Gemini models on this backend** (`claude-*`, `gpt-oss-*`) answer in the Gemini wire per the client; untested here.
...
- **Server tools (architecture.md CR-4, resolved there) — OpenAI Responses carries provider-typed TOOLS; the rest take the documented DEGRADATION.** `Tool::Provider{kind,name,config}` and `Content::ServerToolUse`/`ServerToolResult` (architecture.md §3.1) are opaque passthrough. **OpenAI Responses projects `Tool::Provider` VERBATIM as its native typed tool** (`{type:kind, name?, ...config}` — the canonical serializer's shape, shared with Anthropic; bl-83b4): `image_generation`, `web_search`, `code_interpreter` and the rest reach the wire as the caller wrote them, and a bad `type` is the provider's 400. This is how OpenAI image OUTPUT is requested (CR-Img). Until bl-83b4 Responses REJECTED every provider tool, which made the CR-Img request path this spec described unreachable — found by the first live test. A `Tool::Provider` in `tools[]` → **encode-time `Error{ParseInput}`** (exit 64, "provider-typed tools are not projected for this dialect") on OpenAI chat, Google, and Ollama — fail fast, never a silent drop. A `Content::ServerTool*` block in a replayed transcript → `ParseInput`/64 on OpenAI chat/Responses/Ollama (the existing non-representable-content rejection), and is **dropped** by Google (folded into the `Thinking`/`RedactedThinking` empty-set drop, §4.3). Server-tool RESULT blocks are never decoded/surfaced by these dialects (no such wire block exists — the empty-set rule). Replaying a Responses native tool's OUTPUT item (`image_generation_call`, `web_search_call`) is NOT carried — those items are decoded (the image) or dropped, never replayed; a stateless caller re-sends the image as user input (CR-Img). `Tool::Custom` is unaffected everywhere.
- **CR-Usage — dropped `Usage` detail is DISCARDED today; no `provider_detail`-on-`Usage` field is promised until a consumer exists.** Several dialects report token/duration detail the canonical `Usage` has no slot for: OpenAI Responses `output_tokens_details.reasoning_tokens`/`total_tokens` (§3.5) and Ollama `total_duration`/`eval_duration`/… (§5.7). Canonical `Usage` (architecture.md §3.2) carries only `input_tokens`/`output_tokens`/`cache_read_tokens`/`cache_write_tokens`, the derived `input_total_tokens` (bl-d192) and the carried `context_window` (bl-fb0c) — and **no `provider_detail` field** (that field lives on `CanonicalError`, not `Usage`). Earlier text said this detail "rides `provider_detail`"; that described an **UNBUILT mechanism as if decided** — the kin of "don't store what you can compute": *don't promise what you haven't built*. The honest state: the detail is **dropped, not stashed anywhere**. **Deferred:** no `Usage` field (and no `provider_detail` on `Usage`) is added until a real consumer needs one, at which point it is an additive `v=1` change (architecture.md §3.2) — the same shape in which the server-tool usage counter is scoped (architecture.md §3.3). **Low urgency.**
...
- **Google Cloud Code (Antigravity)** = `mod google_cloudcode` + one arm + one insert + an OPERATOR recipe row (auth.md §11): §4's body and chunk fold reused through `pub(crate)`, wrapped as `{model, request}` on `/v1internal:…` and unwrapped from `{response}`; the entitlement gate is the OAuth row's `user-agent` beta header, not code (§4.10).
