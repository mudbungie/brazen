...
| `extra` | merged top-level / into `options` | the valve: `keep_alive`, `format` (JSON-mode/schema), `options.*` knobs (`num_ctx`, `seed`, `repeat_penalty`, …). Typed fields win (architecture.md §3.1) — per key, since the fold merges an `options` object one level (openai-chat-mapping.md §2.1.1). **`num_ctx` is the context window and has no canonical field**: brazen never states it, so a request runs at the server's own default unless a row pins `body_defaults = { options = { num_ctx = N } }` — which now composes with the typed `max_tokens` the encoder writes as `options.num_predict` instead of being dropped by it (bl-f19d). The two are distinct fields: `num_predict` caps the OUTPUT, `num_ctx` sizes the INPUT window. |
