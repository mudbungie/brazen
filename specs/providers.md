...
| `Message{User, [Document{Base64}]}` | `{type:"input_file", filename:<synth>, file_data:"data:{mt};base64,{data}"}` — data-URI `file_data` (as Chat), with `filename` synthesized `document.{subtype}` from the media type (Responses requires it on `file_data`, openai-chat-mapping.md §2.2 CR-6). **User** turns only. |
| `Message{User, [Document{Url}]}` | `{type:"input_file", file_url:url}` — Responses **fetches web URLs**, so **both** document sources express here, UNLIKE Chat Completions (which rejects the URL, openai-chat-mapping.md §6 CR-6). Documents are **input-only**; no provider returns one, so no decode side (§9 CR-Doc). |
